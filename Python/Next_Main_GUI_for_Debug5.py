...
tk.Button(root, text="[] 全画面", font=("Arial", 10),
          command=lambda: root.event_generate("<F11>"))\
          .grid(row=0, column=6, sticky="ne", padx=5, pady=5)
tk.Button(root, text="X 通常", font=("Arial", 10),
          command=lambda: root.event_generate("<Escape>"))\
          .grid(row=0, column=5, sticky="ne", padx=5, pady=5)
...
def make_entry(label,row,key):
    ...
    tk.Label(root,text=label,font=font_label,bg=bg,fg="white")\
