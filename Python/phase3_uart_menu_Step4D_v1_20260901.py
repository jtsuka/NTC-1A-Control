...
#
#   TC106 Direction-B / Step4D telemetryモニタ:
...
#     を表示する。Step4D v1.3 FIXに従い、short/overlongを分離して診断する。
...
class TcDirectionBMonitor:
    ...
    The rolling buffer is diagnostic/storage state only. It must never hide
    an overlong frame: validity is decided by data_byte_count.
    """
        ...
        for b in data:
            if b != TC_FOOTER:
                ...
                if len(self.data_buf) < self.DATA_LEN:
                    ...
                    # Keep latest five bytes only, but never hide overlength.
                    del self.data_buf[0]
                    self.data_buf.append(b)
                continue
            ...
            # The footer belongs to the frame just evaluated.
            self.clear_partial()
