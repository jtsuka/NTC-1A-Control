...
ch_btn = tk.Button(top_frame, text="[CH1 選択中]", bg="darkblue", fg="white", command=lambda: toggle_channel())
...
row = 0
for ch in (1, 2):
    make_labeled_entry(f"CH{ch} tension(gf)", f"ch{ch}_tension", row); row += 1
    make_labeled_entry(f"CH{ch} length(m)", f"ch{ch}_length", row); row += 1
    make_labeled_entry(f"CH{ch} count()", f"ch{ch}_count", row); row += 1
