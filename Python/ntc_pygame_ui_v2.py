...
# --- 1. ログコールバックのセット (スレッド開始前に必須) ---
log_lines = []
def log_callback(msg):
    log_lines.append(msg)
    if len(log_lines) > 8: log_lines.pop(0)

# スレッドを動かす前に、utils経由で comm 側が使う out を上書きする
...
# --- ボタンとコマンドの定義 ---
buttons = []
# SAFEコマンドを6バイトに修正 (CMD + 4Data形式)
def cmd_safe_on():  send_packet([0xF0, 1, 0, 0, 0])
def cmd_safe_off(): send_packet([0xF0, 0, 0, 0, 0])

btn_x = 50
buttons.append(Button((btn_x, 20, 150, 50), 'SAFE ON', cmd_safe_on))
buttons.append(Button((btn_x + 170, 20, 150, 50), 'SAFE OFF', cmd_safe_off))
...
def on_pad(val):
    ...
    if val.isdigit(): fields[key] += val
        ...
        send_packet([0x01, t & 0xFF, 0, 0, 0]) # 6B送信
...
while True:
    ...
    for idx, key in enumerate(field_keys):
        ...
        pygame.draw.rect(screen, (255,255,255) if idx==current_field else (80,80,80), rect, 2)
