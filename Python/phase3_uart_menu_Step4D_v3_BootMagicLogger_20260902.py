...
TC_FRAME_LEN = 6
TC_FOOTER = 0x7F
ESP_BOOT_MAGIC = bytes.fromhex("DE AD BE EF 01 7F")
...
class TcDirectionBMonitor:
    ...
    def __init__(self):
        ...
        self.frame_count = 0
        self.esp_boot_count = 0
        self._boot_events_pending = []
    ...
    def reset(self):
        ...
        self.frame_count = 0
        self.esp_boot_count = 0
        self._boot_events_pending.clear()
        self.sync_miss_short = 0
        self.sync_miss_overlong = 0

    def take_boot_events(self):
        """Return and clear boot events recognized by feed()."""
        events = self._boot_events_pending[:]
        self._boot_events_pending.clear()
        return events
    ...
    def feed(self, data):
        ...
        for b in data:
            ...
            if self.data_byte_count == self.DATA_LEN:
                ...
                # Step4D local diagnostic boot marker.
                # Detect inside feed() before telemetry frame_count changes.
                if frame == ESP_BOOT_MAGIC:
                    self.esp_boot_count += 1
                    self._boot_events_pending.append(frame)
                else:
                    self.frame_count += 1
                    frames.append(frame)
        ...
        try:
            ...
            if waiting:
                ...
                if tc_enabled_event.is_set():
                    frames = tc_monitor.feed(data)

                    for boot_frame in tc_monitor.take_boot_events():
                        boot_raw = " ".join(f"{b:02X}" for b in boot_frame)
                        boot_text = (
                            f"[ESP BOOT #{tc_monitor.esp_boot_count:06d}] "
                            f"{boot_raw}"
                        )
                        log_line(log_fp, boot_text)
                        print("\n" + boot_text)
