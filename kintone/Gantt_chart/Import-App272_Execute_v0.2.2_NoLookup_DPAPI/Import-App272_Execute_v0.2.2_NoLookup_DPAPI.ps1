...
<#
.SYNOPSIS
  kintone App272 API自動取込 Phase 1B 安全Execute版 v0.2.1 NoLookup

.DESCRIPTION
  - 外注課納期管理CSVを検証する。
  - App270 / App272 をGETし、初回Executeでは既存App272のみ安全更新する。
  ...
  - Execute対象は既存App272の確定済み基幹5項目差分、承認済みBatch日程候補のみ。ルックアップは参照・判定・更新しない。
  - App270追加および新規App272作成はv0.1では禁止する。
  - PUTはGET時の$id＋$revisionを使用し、並行編集があればkintone側で競合停止させる。

.NOTES
  既定はDryRun。書込みには -Execute と -ConfirmExecute 'APP272-EXECUTE' の両方が必要。
  v0.2.2 NoLookup DPAPIはPUTのみ実装し、POST/DELETEは実装しない。App272のルックアップフィールドには依存しない。
#>
...
function Get-DpapiToken {
    ...
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        throw "DPAPI token file not found: $Path"
    }

    try {
        $encrypted = (Get-Content -LiteralPath $Path -Raw).Trim()
        if ([string]::IsNullOrWhiteSpace($encrypted)) {
            throw "encrypted token file is empty: $Path"
        }
        $secure = $encrypted | ConvertTo-SecureString
        $plain = ConvertFrom-SecureStringPlain $secure
        if ([string]::IsNullOrWhiteSpace($plain)) {
            throw "decrypted token is empty: $Path"
        }
        Write-Host ("  {0}: DPAPI stored token ({1})" -f $Label, $Path)
        return $plain
    }
    catch {
        throw "DPAPI token load failed for $Label. Same Windows user/PC? Path=$Path Detail=$($_.Exception.Message)"
    }
}
...
# -------------------- main --------------------
$started = Get-Date
Write-Host '=== App272 API自動取込 Phase 1B Safe Execute v0.2.2 NoLookup DPAPI ===' -ForegroundColor Cyan
...
if ($Execute -and $ConfirmExecute -ne 'APP272-EXECUTE') {
    Write-Error "書込みには -ConfirmExecute 'APP272-EXECUTE' が必要です。"
    exit 31
}
...
if ($MaxWrites -lt 1) {
    Write-Error '-MaxWrites は1以上にしてください。'
    exit 33
}
...
try {
    ...
    else { Write-Host "WRITE             : ENABLED (PUT only / MaxWrites=$MaxWrites)" -ForegroundColor Yellow }
    ...
    if (-not $SkipKintone) {
        ...
        if ([string]::IsNullOrWhiteSpace($ApiToken270)) {
            if ((-not $ForcePromptToken) -and (Test-Path -LiteralPath $token270Path -PathType Leaf)) {
                $ApiToken270 = Get-DpapiToken -Path $token270Path -Label 'App270 READ ONLY'
            } else {
                $ApiToken270 = Get-ApiTokenInteractive 'App270'
            }
        }
        if ([string]::IsNullOrWhiteSpace($ApiToken272)) {
            if ((-not $ForcePromptToken) -and (Test-Path -LiteralPath $token272Path -PathType Leaf)) {
                $ApiToken272 = Get-DpapiToken -Path $token272Path -Label 'App272 PROD'
            } else {
                $ApiToken272 = Get-ApiTokenInteractive 'App272'
            }
        }
    ...
    foreach ($row in $csvRows) {
        ...
        if ($app272Action -eq 'New') {
            $plannedWrite = 'BLOCKED_NEW_APP272'
        }
        elseif ($app270Action -eq 'Add') {
            $plannedWrite = 'BLOCKED_APP270_ADD'
        }
        else {
            $parts = New-Object System.Collections.Generic.List[string]
            if ($app272Action -eq 'Update(BaseOnly)') { $parts.Add('BASE') }
            if ($isScheduleWriteCandidate) { $parts.Add('SCHEDULE') }
            if ($parts.Count -gt 0) { $plannedWrite = 'PUT:' + ($parts -join '+') }
        }
    ...
    # -------------------- Phase 1B preflight / execute --------------------
    $writeCandidates = @($details | Where-Object { $_.WRITE -like 'PUT:*' })
    $blockedNew = @($details | Where-Object WRITE -eq 'BLOCKED_NEW_APP272')
    $blockedApp270Add = @($details | Where-Object WRITE -eq 'BLOCKED_APP270_ADD')
    $writeCountPlanned = $writeCandidates.Count

    if ($Execute) {
        if ($blockedApp270Add.Count -gt 0) {
            throw "安全停止: App270追加候補が $($blockedApp270Add.Count) 件あります。v0.1ではApp270追加を禁止しています。"
        }

        # 新規App272は初回Executeでは書かない。存在しても全体停止はせず、BLOCKEDとしてログに残す。
        if ($writeCountPlanned -gt $MaxWrites) {
            throw "安全停止: PUT予定件数 $writeCountPlanned が MaxWrites=$MaxWrites を超えています。"
        }

        $preflightErrors = New-Object System.Collections.Generic.List[string]

        foreach ($d in $writeCandidates) {
            if (-not [string]::IsNullOrWhiteSpace($d.Errors)) {
                $preflightErrors.Add("Row $($d.Row): Errors=$($d.Errors)")
            }
            if ([string]::IsNullOrWhiteSpace([string]$d.App272RecordId) -or
                [string]::IsNullOrWhiteSpace([string]$d.App272Revision)) {
                $preflightErrors.Add("Row $($d.Row): App272 record id/revision missing")
            }
            if ($d.WRITE -like '*SCHEDULE*') {
                if ($d.RuleStatus -ne '使用') {
                    $preflightErrors.Add("Row $($d.Row): schedule write but RuleStatus=$($d.RuleStatus)")
                }
                if ($d.K日程種別_候補 -ne 'Batch') {
                    $preflightErrors.Add("Row $($d.Row): K日程種別候補 is not Batch")
                }
                if ([string]::IsNullOrWhiteSpace([string]$d.K加工着手日_候補) -or
                    [string]::IsNullOrWhiteSpace([string]$d.K完成検査日_候補)) {
                    $preflightErrors.Add("Row $($d.Row): schedule candidate date missing")
                }
                if ($d.K計算根拠_候補 -notin @('Company','Provisional')) {
                    $preflightErrors.Add("Row $($d.Row): invalid K計算根拠候補=$($d.K計算根拠_候補)")
                }
            }
        }

        if ($writeCandidates | Where-Object { $_.WRITE -like '*SCHEDULE*' } | Select-Object -First 1) {
            if (-not $kCalculationBasisFieldExists) {
                $preflightErrors.Add('K計算根拠 field is missing in App272')
            }
        }

        if ($preflightErrors.Count -gt 0) {
            throw "安全停止: Execute前検証NG:`r`n - " + ($preflightErrors -join "`r`n - ")
        }

        Write-Host ''
        Write-Host "=== EXECUTE START: PUT予定 $writeCountPlanned 件 ===" -ForegroundColor Yellow
        if ($blockedNew.Count -gt 0) {
            Write-Warning "新規App272 $($blockedNew.Count) 件はv0.1では書込みません。BLOCKED_NEW_APP272としてログに残します。"
        }
        ...
        foreach ($d in $writeCandidates) {
            ...
            if ($d.WRITE -like '*BASE*') {
                $diffs = @()
                if (-not [string]::IsNullOrWhiteSpace([string]$d.BaseDiffFields)) {
                    $diffs = @(([string]$d.BaseDiffFields).Split(',') | ForEach-Object { $_.Trim() } | Where-Object { $_ })
                }

                foreach ($field in $diffs) {
                    switch ($field) {
                        'ODERNO'      { $recordFields['ODERNO'] = @{ value = [string]$d.ODERNO } }
                        'VENDOR_TEXT' { $recordFields['VENDOR_TEXT'] = @{ value = [string]$d.指示先名 } }
                        'VENDOR_DD'   { $recordFields['VENDOR_DD'] = @{ value = [string]$d.指示先名 } }
                        '日付'        { $recordFields['日付'] = @{ value = [string]$d.手配日 } }
                        '日付_1'      { $recordFields['日付_1'] = @{ value = [string]$d.納期 } }
                        default       { throw "安全停止: Row $($d.Row) 未許可BaseDiffFields=$field" }
                    }
                }
            }
            ...
            if ($d.WRITE -like '*SCHEDULE*') {
                ...
                $recordFields['K日程種別'] = @{ value = 'Batch' }
                $recordFields['K暫定設定'] = @{ value = [string]$d.K暫定設定_候補 }
                $recordFields['K計算根拠'] = @{ value = [string]$d.K計算根拠_候補 }
            }

            if ($recordFields.Count -eq 0) {
                throw "安全停止: Row $($d.Row) PUT対象フィールドが0件です。"
            }

            Write-Host ("PUT {0}/{1} Row={2} PO={3} Fields={4}" -f `
                ($executed + 1), $writeCountPlanned, $d.Row, $d.発注番号, (($recordFields.Keys | Sort-Object) -join ','))

            $null = Invoke-KintoneUpdateRecord `
                -BaseUrl $BaseUrl `
                -AppId $App272 `
                -Token $ApiToken272 `
                -RecordId ([string]$d.App272RecordId) `
                -Revision ([string]$d.App272Revision) `
                -RecordFields $recordFields

            $d.WRITE = 'DONE:' + $d.WRITE.Substring(4)
            $executed++
        }

        Write-Host "=== EXECUTE COMPLETE: $executed 件 ===" -ForegroundColor Green
    }

    $stamp = (Get-Date).ToString('yyyyMMdd_HHmmss')
    $modeName = $(if ($Execute) { "Execute" } else { "DryRun" })
    $detailPath = Join-Path $LogDirectory "Import-App272_${modeName}_Detail_$stamp.csv"
    $summaryPath = Join-Path $LogDirectory "Import-App272_${modeName}_Summary_$stamp.txt"
...
Lookup handling               : DISABLED (NoLookup)
...
Planned PUT writes            : $writeCountPlanned
Blocked New App272            : $($blockedNew.Count)
MaxWrites                     : $MaxWrites
WRITE                         : $(if ($Execute) { 'PUT executed for allowed existing App272 only' } else { 'NONE' })
