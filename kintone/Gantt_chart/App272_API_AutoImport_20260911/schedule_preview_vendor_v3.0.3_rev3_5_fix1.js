(function () {
  ...
  /*
   * 外注先別 暫定日程設定 v3.0.3
   * 対応仕様：コードレビュー仕様書 Rev.3.5 (2026-09-08)
   ...
   * - 旧v2.1の全件復元は使用せず、選択外注先＋Auto（空欄はAuto相当）に限定して基準日程へ戻す。
   */
    ...
    onButton.onclick = async function () {
      ...
      try {
        ...
        const message = [
          ...
          `既存OFF/空欄からON対象：${result.provisionalOffOrBlank}件`,
    ...
    offButton.onclick = async function () {
      ...
      try {
        ...
        const message = [
          ...
          `K暫定設定→OFF補正：${result.provisionalNormalized}件`,
          `K日程種別→Auto補正：${result.scheduleTypeNormalized}件`,
          '',
  ...
  function buildApplyUpdates(records, selectedVendor, offsets) {
    ...
    records.forEach(function (record) {
      ...
      if (scheduleType === CONFIG.manualValue) {
        result.manualProtected++;
        return;
      }

      // 既存データの空欄はフォーム初期値相当のAutoとして扱う。
      // Autoまたは空欄だけを自動処理対象とし、更新時にAutoへ正規化する。
      if (scheduleType && scheduleType !== CONFIG.autoValue) {
        result.manualProtected++;
        return;
      }

      const currentProvisional = getValue(record, CONFIG.provisionalEnabledField);
      if (currentProvisional !== CONFIG.provisionalOnValue) {
        result.provisionalOffOrBlank++;
      }
      ...
      result.updates.push({
        ...
        record: {
          ...
          [CONFIG.provisionalEnabledField]: { value: CONFIG.provisionalOnValue },
  ...
  function buildRestoreUpdates(records, selectedVendor) {
    const result = {
      ...
      alreadyBaseline: 0,
      missingBaseDate: 0,
      provisionalNormalized: 0,
      scheduleTypeNormalized: 0
    };
    ...
    records.forEach(function (record) {
      ...
      const needsDateRestore =
        currentStart !== orderDateValue ||
        currentInspection !== dueValue;

      const needsProvisionalNormalize =
        currentProvisional !== CONFIG.provisionalOffValue;

      const needsScheduleTypeNormalize =
        scheduleType !== CONFIG.autoValue;

      if (!needsDateRestore && !needsProvisionalNormalize && !needsScheduleTypeNormalize) {
        result.alreadyBaseline++;
        return;
      }

      if (needsProvisionalNormalize) result.provisionalNormalized++;
      if (needsScheduleTypeNormalize) result.scheduleTypeNormalized++;
      ...
      result.updates.push({
        ...
        record: {
          ...
          [CONFIG.provisionalEnabledField]: { value: CONFIG.provisionalOffValue },
          [CONFIG.scheduleTypeField]: { value: CONFIG.autoValue }
        }
      });
    });
