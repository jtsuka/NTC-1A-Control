...
#
# 処理内容:
#   1. 日付列: 基幹CSVの "yy-MM-dd" は 20yy と明示解釈し、出力を "yyyy-MM-dd" に統一
...
# パスを安全に絶対パス化（絶対パス/UNCはそのまま、相対パスのみカレント基準）
...
$InputPath  = ConvertTo-FullPath $InputPath
$OutputPath = ConvertTo-FullPath $OutputPath

# ---- Safety preflight ----
if (-not (Test-Path -LiteralPath $InputPath -PathType Leaf)) {
    Write-Error "入力CSVが見つかりません: $InputPath"
    exit 1
}
...
if ([string]::Equals(
    ...
    Write-Error "InputPath と OutputPath に同一ファイルは指定できません: $InputPath"
    exit 1
}
...
if ([string]::IsNullOrWhiteSpace($outputDir) -or
    ...
    Write-Error "出力先フォルダが見つかりません: $outputDir"
    exit 1
}
    ...
    try {
        ...
        for ($i = 0; $i -lt $fields.Count; $i++) {
            ...
            if ($dateIndexes.ContainsKey($i) -and $val -ne "") {
                ...
                # 基幹CSV正式仕様: yy-MM-dd は 20yy-MM-dd と明示変換する。
                # .NET / OS の TwoDigitYearMax には依存しない。
                if ($rawDate -match '^(\d{2})-(\d{2})-(\d{2})$') {
                    $expandedDate = '20' + $rawDate
                ...
                if ($ok) {
                    $newVal = $parsed.ToString("yyyy-MM-dd")
                    if ($newVal -ne $val) {
                        $fields[$i] = $newVal
                        $dateConverted++
                    }
