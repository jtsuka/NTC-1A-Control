...
<#
.SYNOPSIS
  kintone App272 / App270 API自動取込 Phase 1A DryRun専用版 v1.3

.DESCRIPTION
  - 外注課納期管理CSVを検証する。
  - App270 / App272 は GET のみ実行し、追加・更新・削除は一切行わない。
  ...
  - Phase 1Aでは既存K日程は種別に関係なく保護し、優先度ルールによる上書きはまだ実行しない。

.NOTES
  この版には kintone 書込みAPI (POST/PUT/DELETE) を実装していない。
  したがって -DryRun 以外では起動しない。
#>
...
# -------------------- main --------------------
$started = Get-Date
Write-Host '=== App272 API自動取込 Phase 1A DryRun v1.3 ===' -ForegroundColor Cyan

if (-not $DryRun) {
    Write-Error 'この版はDryRun専用です。必ず -DryRun を指定してください。'
    exit 30
}
...
try {
    ...
    Write-Host 'WRITE             : NONE (GET only)'
    ...
    $detailPath = Join-Path $LogDirectory "Import-App272_DryRun_Detail_$stamp.csv"
    $summaryPath = Join-Path $LogDirectory "Import-App272_DryRun_Summary_$stamp.txt"
...
"@
    ...
    if (-not $kCalculationBasisFieldExists -and -not $SkipKintone) {
        Add-Warning 'App272に K計算根拠 フィールドはまだ確認できません。DryRunには影響しません。Execute前に追加してください。'
    }
