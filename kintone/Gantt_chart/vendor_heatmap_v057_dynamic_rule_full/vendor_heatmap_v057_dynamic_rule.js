/*
 * 外注課 外注先負荷ヒートマップ / 日程リスク可視化 Ver0.5.0
      ...
      return `
        <div class="eh-gantt-row">
          ...
          <div class="eh-track">
            ...
            ${workbar ? `<div class="eh-workbar eh-plugin-clip eh-work-${risk.severity}${workBad ? ' eh-alert-segment' : ''}" style="${cellSpanStyle(workbar, 2)}" title="${escapeAttr(r.product || '')}"><span class="eh-plugin-start-cap"></span><span class="eh-workbar-label">${escapeHtml(r.product || '')}</span><span class="eh-plugin-end-cap"></span></div>` : ''}
    ...
    return `
      <section class="eh-section">
        ...
        <div class="eh-schedule-note">
          1日1マスの予定表表示です。下段は標準プラグイン風の絶対配置バーで試作表示しています。
        </div>
