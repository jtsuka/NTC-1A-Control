/*
 * 外注課 外注先負荷ヒートマップ / 日程リスク可視化 Ver0.6.1.8.2
    ...
    const rows = riskRows.map(x => {
      ...
      return `
        <tr>
          ...
          <td><a class="eh-record-link${r.arrangementNo ? '' : ' eh-no-arrangement'}" href="${recordUrl(r.id)}">${escapeHtml(r.displayKey)}</a></td>
          <td>${r.orderSlipNo ? escapeHtml(r.orderSlipNo) : '<span class="eh-muted">—</span>'}</td>
        ...
        <div class="eh-gantt-row${r.isManual ? ' eh-manual-row' : ''}${r.receiptDate ? ' eh-completed' : ''}" data-record-id="${escapeAttr(r.id)}">
          <div class="eh-gantt-cell eh-gantt-col-order">
            <div class="eh-order-key-line">
              ...
              <a href="${recordUrl(r.id)}" class="eh-record-link${r.arrangementNo ? '' : ' eh-no-arrangement'}" title="手配書番号: ${escapeAttr(displayKey)}">${escapeHtml(displayKey)}</a>
            </div>
            <div class="eh-order-slip-no" title="注文書番号">注文書 ${r.orderSlipNo ? escapeHtml(r.orderSlipNo) : '—'}</div>
