...
.DESCRIPTION
  ...
  Executeモードでは次の順で処理する:
    ...
    9. Phase1C最終DryRun = POST可能New 0件 / App270未登録保留集合不変を確認
   10. NoLookup最終DryRun = Updates=0 / Errors=0 / Warnings=0 / BlockedNew=App270未登録保留件数を確認
   11. 正常時のみTMP削除
...
param(
    ...
    [string]$NewPostScript = 'C:\HPDB\Import-App272_Phase1C_New_v0.7.6_NoLookup_CANDIDATE_REVIEW.ps1',
...
function Get-Phase1CPlan {
    ...
    $newRows = @($rows | Where-Object { $_.App272 -eq 'New' })
    $postRows = @($rows | Where-Object { ([string]$_.WRITE) -eq 'POST:NEW' })
    $deferredRows = @($rows | Where-Object { ([string]$_.WRITE) -eq 'BLOCKED_APP270_ADD' })

    function Get-UniquePoSet([object[]]$InputRows,[string]$Label){
        $po = @($InputRows | ForEach-Object { ([string]$_.発注番号).Trim() } | Where-Object { $_ } | Sort-Object -Unique)
        if($po.Count -ne $InputRows.Count){
            Stop-Safety "$Label PO set is not unique. Rows=$($InputRows.Count) UniquePO=$($po.Count)"
        }
        return $po
    }

    $newPo = @(Get-UniquePoSet $newRows 'Phase1C New')
    $postPo = @(Get-UniquePoSet $postRows 'Phase1C POST eligible')
    $deferredPo = @(Get-UniquePoSet $deferredRows 'Phase1C App270-deferred')

    $otherBlocked = @($newRows | Where-Object {
        ([string]$_.WRITE) -ne 'POST:NEW' -and ([string]$_.WRITE) -ne 'BLOCKED_APP270_ADD'
    })
    if($otherBlocked.Count -ne 0){
        $kinds = @($otherBlocked | ForEach-Object { [string]$_.WRITE } | Sort-Object -Unique) -join ','
        Stop-Safety "Phase1C has unexpected blocked New rows=$($otherBlocked.Count) WRITE=$kinds"
    }
    if(($postRows.Count + $deferredRows.Count) -ne $newRows.Count){
        Stop-Safety "Phase1C New partition mismatch. New=$($newRows.Count) Post=$($postRows.Count) Deferred=$($deferredRows.Count)"
    }
    if($postPo.Count -gt $MaxCreates){
        Stop-Safety "Phase1C planned POST=$($postPo.Count) exceeds MaxNew/MaxCreates=$MaxCreates"
    }
    ...
    return [pscustomobject]@{
        ...
        PostPo = $postPo
        PostCount = $postPo.Count
        DeferredPo = $deferredPo
        DeferredCount = $deferredPo.Count
    }
}
function Get-PhaseBPlan {
    param(
        ...
        [string]$DateTag,
        [string]$LogDir,
        [Nullable[int]]$ExpectedBlockedNew = $null
    )
...
try {
    ...
    $putPlan = Get-PhaseBPlan `
        ...
        -DateTag $DateTag `
        -LogDir $LogDir `
        -ExpectedBlockedNew $newPlan.NewCount

    Write-Host ''
    Write-Host '=== PRE-FLIGHT SUMMARY ==='
    Write-Host ("NEW candidates   : {0}" -f $newPlan.NewCount)
    Write-Host ("POST eligible    : {0}" -f $newPlan.PostCount)
    Write-Host ("Deferred(App270) : {0}" -f $newPlan.DeferredCount)
    ...
    # EXECUTE NEW POST
    # ------------------------------------------------------------
    if($newPlan.PostCount -gt 0){
        Write-Host ''
        Write-Host ("[E1] Phase1C Execute POST eligible={0} / deferred={1}..." -f $newPlan.PostCount,$newPlan.DeferredCount)
        & $NewPostScript `
            -CsvPath $formatted `
            -Environment PROD `
            -App272 272 `
            ...
            -ApiToken272 $token272 `
            -Execute `
            -ExpectedNewPo $newPlan.PostPo `
            -MaxCreates $newPlan.PostCount `
        ...
        Write-Host ''
        Write-Host '[E1] Phase1C Execute skipped: POST eligible=0'
    }
    ...
    $putPlan2 = Get-PhaseBPlan `
        ...
        -DateTag $DateTag `
        -LogDir $LogDir `
        -ExpectedBlockedNew $newPlan.DeferredCount
    ...
    if($finalNew.PostCount -ne 0){
        Stop-Safety "Final verification failed: POST-eligible NEW remains $($finalNew.PostCount)"
    }
    if($finalNew.DeferredCount -ne $newPlan.DeferredCount){
        Stop-Safety "Final verification failed: deferred count changed. Initial=$($newPlan.DeferredCount) Final=$($finalNew.DeferredCount)"
    }
    $deferredDiff = @(Compare-Object -ReferenceObject @($newPlan.DeferredPo | Sort-Object) -DifferenceObject @($finalNew.DeferredPo | Sort-Object))
    if($deferredDiff.Count -ne 0){
        $diffText = ($deferredDiff | ForEach-Object { "$($_.InputObject)[$($_.SideIndicator)]" }) -join ','
        Stop-Safety "Final verification failed: deferred PO set changed: $diffText"
    }
    ...
    if($finalPut.UpdateCount -ne 0 -or $finalPut.ErrorCount -ne 0 -or $finalPut.WarningCount -ne 0 -or $finalPut.BlockedNewCount -ne $newPlan.DeferredCount){
    ...
    Write-Host '  Phase1C POST eligible : 0 remaining'
    Write-Host ("  Phase1C Deferred(App270): {0} unchanged" -f $newPlan.DeferredCount)
