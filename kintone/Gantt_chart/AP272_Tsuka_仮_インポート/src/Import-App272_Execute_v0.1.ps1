...
<#
.SYNOPSIS
  kintone App272 API自動取込 Phase 1B 安全Execute版 v0.1
...
.DESCRIPTION
  ...
  - Execute対象は既存App272の確定済み基幹5項目差分、既存Lookup同期、承認済みBatch日程候補のみ。
...
.NOTES
  ...
  v0.1はPUTのみ実装し、POST/DELETEは実装しない。
#>
...
# -------------------- main --------------------
$started = Get-Date
Write-Host '=== App272 API自動取込 Phase 1B Safe Execute v0.1 ===' -ForegroundColor Cyan
...
try {
    ...
    foreach ($row in $csvRows) {
        ...
        elseif ($app270Action -eq 'Add') {
            ...
            if ($lookupAction -eq 'Sync') { $parts.Add('LOOKUP') }
    ...
    if ($Execute) {
        ...
        foreach ($d in $writeCandidates) {
            ...
            if ($d.WRITE -like '*LOOKUP*') {
                $recordFields['ルックアップ'] = @{ value = [string]$d.ODERNO }
            }
