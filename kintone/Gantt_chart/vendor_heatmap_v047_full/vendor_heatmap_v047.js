/*
 * 外注課 外注先負荷ヒートマップ / 日程リスク可視化 Ver0.4.7
...
(function () {
    ...
    return `
      <section class="eh-section">
        <div class="eh-section-title eh-detail-title">
          ...
          <label class="eh-detail-size-control">
            表示サイズ
            <select id="eh-detail-size">
  ...
  function bindEvents(root) {
    ...
    const detailSize = root.querySelector('#eh-detail-size');
    if (detailSize) {
      detailSize.addEventListener('change', function () {
        ...
        render(root);
        const detail = root.querySelector('.eh-detail-title');
