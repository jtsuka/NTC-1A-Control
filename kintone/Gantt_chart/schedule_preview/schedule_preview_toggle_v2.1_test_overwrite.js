(function () {
  ...
  kintone.events.on('app.record.index.show', function (event) {
    ...
    if (storedState === 'on') {
      ...
      setInputsDisabled(inspectionInput, startInput, false);
    }
      ...
      try {
        ...
        if (currentState === 'on') {
          ...
          const message = [
            ...
            `基準日程へ復元予定：${result.updates.length}件`,
            '',
            '暫定OFFでは、全案件を「K加工着手日＝手配日 / K完成検査日＝納期」へ戻します。',
            '実行しますか？'
          ].join('\n');
  ...
  function setButtonState(button, state) {
    ...
    if (state === 'on') {
      ...
      button.title = '赤＝暫定日程ON。入力値は編集できます。クリックすると基準日程へ戻します。';
    } else {
  ...
  function buildApplyUpdates(records, offsets) {
    ...
    records.forEach(function (record) {
      // TEST運用：
      // 既存のK加工着手日 / K完成検査日は保護せず、
      // 入力された「検査N日前 / 着手N日前」で上書きする。
