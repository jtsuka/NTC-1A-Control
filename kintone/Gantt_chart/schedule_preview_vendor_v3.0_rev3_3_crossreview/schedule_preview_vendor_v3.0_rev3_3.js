(function () {
  ...
  kintone.events.on('app.record.index.show', function (event) {
    ...
    const offButton = createActionButton(
      ...
      '暫定OFF',
      '#2f6fdd',
      '選択外注先の K日程種別=Auto のK日付だけを空欄にする（Manualは保護）'
    );
    ...
    offButton.onclick = async function () {
      ...
      try {
        const appId = kintone.app.getId();
        const records = await fetchAllRecords(appId);
        const result = buildClearUpdates(records, vendor);
        ...
        const message = [
          ...
          'Auto日程を解除します。',
          '',
          `クリア予定：${result.updates.length}件`,
          `Manual保護：${result.manualProtected}件`,
          `既に空欄：${result.alreadyBlank}件`,
          '',
          `${CONFIG.startField}・${CONFIG.inspectionField}を空欄にします。`,
          'Manual案件は変更しません。',
          '',
          '実行しますか？'
        ].join('\n');
  ...
  function buildClearUpdates(records, selectedVendor) {
    const result = {
      updates: [],
      vendorMismatch: 0,
      manualProtected: 0,
    ...
    records.forEach(function (record) {
      ...
      if (!currentStart && !currentInspection) {
        result.alreadyBlank++;
        return;
      }
      ...
      result.updates.push({
        ...
        revision: record.$revision.value,
        record: {
          [CONFIG.startField]: { value: null },
          [CONFIG.inspectionField]: { value: null },
          [CONFIG.scheduleTypeField]: { value: CONFIG.autoValue }
        }
      });
    });
