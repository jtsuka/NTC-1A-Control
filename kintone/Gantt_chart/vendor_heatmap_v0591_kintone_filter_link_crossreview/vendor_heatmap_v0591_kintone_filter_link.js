/*
 * 外注課 外注先負荷ヒートマップ / 日程リスク可視化 Ver0.5.9.1
...
(function () {
  ...
  async function fetchActiveRecords(listConditionArg) {
     ...
     * このヒートマップ用に再取得できる。
     *
     * 既存仕様の「納期が今日以降」を固定条件として維持する。
     * したがって、標準一覧の絞り込み結果に過去納期案件が含まれる場合、
     * kintone一覧の表示件数とヒートマップの「対象件数」は一致しないことがある。
     */
    const listCondition =
    ...
    while (true) {
      ...
      // Ver0.5.8までの既存対象条件を維持
      conditions.push(`${f.dueDate} >= "${today}"`);
  ...
  function renderVendorDetail(records, vendor, viewStart, viewEnd) {
    ...
    const rows = list.map(r => {
      ...
      return `
        <div class="eh-gantt-row${r.isManual ? ' eh-manual-row' : ''}">
          <div class="eh-gantt-info">
            ${r.isManual ? '<span class="eh-manual-badge" title="手動調整された日程">M</span>' : ''}<a href="${recordUrl(r.id)}" class="eh-record-link">${escapeHtml(r.key || ('#' + r.id))}</a>
    ...
    return `
      <section class="eh-section">
        ...
        <div class="eh-schedule-note">
          1日1マスの予定表表示です。M表示・太枠・背景色変更は手動調整（K日程種別=Manual）を示します。
        </div>
