/*
 * 外注課 外注先負荷ヒートマップ / 日程リスク可視化 Ver0.3.1
    ...
    return `
      <div class="eh-header">
        <div>
          ...
          <div class="eh-legend">
            <span class="eh-legend-item"><span class="eh-legend-order"></span>手配日</span>
            <span class="eh-legend-item"><span class="eh-legend-start"></span>仕掛日</span>
            <span class="eh-legend-bar"></span>
            <span class="eh-legend-item"><span class="eh-legend-inspection"></span>検収日</span>
            <span class="eh-legend-item"><span class="eh-legend-due"></span>納期</span>
      ...
      return `
        <div class="eh-gantt-row">
          ...
          <div class="eh-track">
            ...
            ${workbar ? `<div class="eh-workbar eh-work-${risk.severity}${workBad ? ' eh-alert-segment' : ''}" style="left:${workbar.left}%;width:${workbar.width}%"></div>` : ''}
            ...
            ${makeMarker(r.orderDate, viewStart, viewEnd, totalDays, `eh-order-marker${leftBad ? ' eh-alert-marker' : ''}`, '手配日')}
            ${makeMarker(r.startDate, viewStart, viewEnd, totalDays, `eh-start-marker${leftBad || workBad ? ' eh-alert-marker' : ''}`, '仕掛日')}
            ${makeMarker(r.inspectionDate, viewStart, viewEnd, totalDays, `eh-inspection-marker${workBad || rightBad ? ' eh-alert-marker' : ''}`, '検収日')}
            ${makeMarker(r.dueDate, viewStart, viewEnd, totalDays, `eh-due-marker${rightBad ? ' eh-alert-marker' : ''}`, '納期')}
          </div>
        </div>
      `;
    }).join('');
    ...
    return `
      <section class="eh-section">
        <div class="eh-section-title">
          ...
          <span>外注先行をクリックするとここが切り替わります。案件番号クリックで標準レコード画面へ。</span>
        </div>
        <div class="eh-gantt-scale">
