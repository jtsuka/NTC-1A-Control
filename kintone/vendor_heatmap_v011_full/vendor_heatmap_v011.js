/*
 * 外注課 外注先負荷ヒートマップ / 日程リスク可視化 Ver0.1.1
...
(function () {
      ...
      <div class="eh-header">
        <div>
          ...
          <div class="eh-subtitle">
            注文日～納期を細線、K加工着手日～K完成検査日を太線で表示します。
  ...
  function renderHeatmap(vendorStats, dates) {
    ...
    const rows = vendorStats.map(v => {
      ...
      const cells = v.counts.map((count, i) => {
        ...
        return `<td class="eh-heat eh-${level}" title="${escapeHtml(title)}">${count || ''}</td>`;
      }).join('');
  ...
  function renderVendorDetail(records, vendor, viewStart, viewEnd) {
    ...
    const rows = list.map(r => {
      const baseline = clippedSpan(r.orderDate, r.dueDate, viewStart, viewEnd, totalDays);
      ...
      return `
        <div class="eh-gantt-row">
          ...
          <div class="eh-track">
            ${baseline ? `<div class="eh-baseline" style="left:${baseline.left}%;width:${baseline.width}%"></div>` : ''}
            ${workbar ? `<div class="eh-workbar eh-work-${risk.severity}" style="left:${workbar.left}%;width:${workbar.width}%"></div>` : ''}
            ${makeMarker(r.orderDate, viewStart, viewEnd, totalDays, 'eh-order-marker', '注文日')}
            ${makeMarker(r.dueDate, viewStart, viewEnd, totalDays, 'eh-due-marker', '納期')}
          </div>
        </div>
      `;
    }).join('');
