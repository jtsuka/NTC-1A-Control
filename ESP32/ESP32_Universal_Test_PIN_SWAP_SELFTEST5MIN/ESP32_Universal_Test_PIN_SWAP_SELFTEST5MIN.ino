...
// ========================================================
// Self Test Mode (Nano Every echo-back) for ~5 minutes
// 1: run self-test only (UART 300bps on GPIO1/2)
// 0: normal bridge mode (BitBang 9-bit)
// ========================================================
#define SELF_TEST_MODE 1
#define SELF_TEST_DURATION_MS (5UL * 60UL * 1000UL)
#define SELF_TEST_INTERVAL_MS 500
...
HardwareSerial SerialTC(2); // Self-test link to Nano Every (GPIO1/2)
...
// ========================================================
// Self-test (ESP32-S3 <-> Nano Every echo-back)
// - Uses HW UART (SerialTC) at 300bps, 8N1
// - Sends 6-byte test packets, expects identical echo
// ========================================================
#if SELF_TEST_MODE
static void logBoth(const String &s) {
  Serial.println(s);
  if (SerialPi) SerialPi.println(s);
}

static bool readExact(HardwareSerial &ser, uint8_t *buf, size_t n, uint32_t timeout_ms) {
  size_t got = 0;
  uint32_t t0 = millis();
  while (got < n) {
...
static uint8_t checksum7_5(const uint8_t *d5) {
  uint16_t s = 0;
  for (int i = 0; i < 5; i++) s += d5[i];
  return (uint8_t)(s & 0x7F);
}

static void dumpHexLine(const uint8_t *p, size_t n, String &out) {
  out.reserve(n * 3 + 4);
...
static void runSelfTest5min() {
  logBoth("=== SELF TEST MODE: Nano Every echo-back (6 bytes) ===");
  logBoth("Link: GPIO1(TX) / GPIO2(RX) @ 300bps 8N1, duration ~5min");

  // Enable level shifter banks
  digitalWrite(PIN_LSHIFT_OE1, LOW);
  digitalWrite(PIN_LSHIFT_OE2, LOW);

  // Start Pi UART (optional logging)
  SerialPi.begin(PI_BAUD, SERIAL_8N1, PIN_PI_UART_RX, PIN_PI_UART_TX);

  // Start Nano link
  SerialTC.begin(TC_BAUD, SERIAL_8N1, PIN_TC_RX, PIN_TC_TX);
  SerialTC.setTimeout(0);

  uint32_t t_end = millis() + (uint32_t)SELF_TEST_DURATION_MS;

  uint32_t sent = 0, ok = 0, timeout = 0, mismatch = 0, badchk = 0;
  ...
  while ((int32_t)(millis() - t_end) < 0) {
    // clear RX garbage
    while (SerialTC.available() > 0) (void)SerialTC.read();

    // packet: [0]=0xA5 [1]=seq [2]=0x5A [3]=0x00 [4]=0xFF [5]=checksum7(sum of first 5)
    tx[0] = 0xA5;
    tx[1] = seq++;
    tx[2] = 0x5A;
    tx[3] = 0x00;
    ...
    bool got = readExact(SerialTC, rx, 6, 700); // 300bpsなので余裕を持つ
    if (!got) {
      timeout++;
    } else {
      // checksum check (optional)
      if (rx[5] != checksum7_5(rx)) badchk++;
    ...
    // periodic log (every ~5 sec)
    if ((sent % 10) == 0) {
      String s = "sent=" + String(sent) + " ok=" + String(ok) +
                 " timeout=" + String(timeout) + " mismatch=" + String(mismatch) +
                 " badchk=" + String(badchk);
      logBoth(s);
    }

    delay(SELF_TEST_INTERVAL_MS);
  }

  String summary = "=== SELF TEST DONE === sent=" + String(sent) +
                   " ok=" + String(ok) +
  ...
  // show last packet (for quick eyeballing)
  String txs="last TX: "; dumpHexLine(tx,6,txs);
  String rxs="last RX: "; dumpHexLine(rx,6,rxs);
  logBoth(txs);
  logBoth(rxs);

  logBoth("HALT (self-test only). Reflash with SELF_TEST_MODE=0 to return to bridge.");
}
#endif
