...
// Step4D CLOSE diagnostic:
// Buffer the first 40 C3-valid-frame processing timestamps in RAM.
// Per-frame C3/PiTask-TX Serial output is suppressed during capture so
// the diagnostic itself does not distort the timing window.
static constexpr uint8_t C4_BOOT_TS_COUNT = 40;
static uint32_t c4BootFrameTs[C4_BOOT_TS_COUNT]{};
static uint8_t  c4BootFrameTsUsed = 0;
static bool     c4BootTsDumped = false;

static uint32_t c4DiagSetupStartUs      = 0;
static uint32_t c4DiagInterruptAttachUs = 0;
static uint32_t c4DiagObjectsReadyUs    = 0;
static uint32_t c4DiagTasksCreatedUs    = 0;
static uint32_t c4DiagTcTaskStartUs     = 0;
static uint32_t c4DiagPiTaskStartUs     = 0;

// USB CDC Serial blocking diagnostic.
// Record before/after micros() around startup Serial.println() calls.
// Do not print these measurements until after the first 40 valid frames
// have been captured, so the measurement does not add startup Serial traffic.
static constexpr uint8_t C4_SERIAL_TC_PRINTS = 5;
static uint32_t c4TcSerialBefore[C4_SERIAL_TC_PRINTS]{};
static uint32_t c4TcSerialAfter[C4_SERIAL_TC_PRINTS]{};
static uint32_t c4PiSerialBefore = 0;
static uint32_t c4PiSerialAfter  = 0;

static bool c4BootTimingCaptureActive() {
  return c4BootFrameTsUsed < C4_BOOT_TS_COUNT;
}

static void c4DiagRecordFrameTimestamp() {
  if (c4BootFrameTsUsed < C4_BOOT_TS_COUNT) {
    c4BootFrameTs[c4BootFrameTsUsed++] = micros();
  }
}

static void c4DiagDumpBootTimingOnce() {
  if (c4BootTsDumped || c4BootFrameTsUsed < C4_BOOT_TS_COUNT) {
    return;
  }
  ...
  Serial.println();
  Serial.println("=== C4 BOOT TIMESTAMP DIAG ===");
  Serial.print("setup_start_us=");      Serial.println(c4DiagSetupStartUs);
  Serial.print("interrupt_attach_us="); Serial.println(c4DiagInterruptAttachUs);
  Serial.print("objects_ready_us=");    Serial.println(c4DiagObjectsReadyUs);
  Serial.print("tasks_created_us=");    Serial.println(c4DiagTasksCreatedUs);
  Serial.print("tc_task_start_us=");    Serial.println(c4DiagTcTaskStartUs);
  Serial.print("pi_task_start_us=");    Serial.println(c4DiagPiTaskStartUs);

  Serial.println("--- USB CDC Serial.println timing ---");
  for (uint8_t i = 0; i < C4_SERIAL_TC_PRINTS; i++) {
    Serial.print("TcPrint");
    Serial.print(i + 1);
    Serial.print(" before_us=");
    Serial.print(c4TcSerialBefore[i]);
    Serial.print(" after_us=");
    Serial.print(c4TcSerialAfter[i]);
    Serial.print(" dt_us=");
    Serial.println(c4TcSerialAfter[i] - c4TcSerialBefore[i]);
  }

  Serial.print("PiPrint1 before_us=");
  Serial.print(c4PiSerialBefore);
  Serial.print(" after_us=");
  Serial.print(c4PiSerialAfter);
  Serial.print(" dt_us=");
  Serial.println(c4PiSerialAfter - c4PiSerialBefore);

  for (uint8_t i = 0; i < C4_BOOT_TS_COUNT; i++) {
    Serial.print("F");
    if (i + 1 < 10) Serial.print('0');
    Serial.print(i + 1);
    Serial.print(" t_us=");
    Serial.print(c4BootFrameTs[i]);

    if (i == 0) {
      Serial.println(" dt_us=NA");
    } else {
      const uint32_t dt = c4BootFrameTs[i] - c4BootFrameTs[i - 1];
      Serial.print(" dt_us=");
      Serial.println(dt);
    }
  }
  ...
  const uint32_t span =
    c4BootFrameTs[C4_BOOT_TS_COUNT - 1] - c4BootFrameTs[0];

  Serial.print("F01_to_F40_span_us=");
  Serial.println(span);
  Serial.println("=== END C4 BOOT TIMESTAMP DIAG ===");
  Serial.println();
}
#endif
...
#if TC_TEST_MODE == 8
      // Evaluate capture state before recording this frame.
      // This keeps F01..F40 fully silent on the live Serial log.
      const bool suppressLiveFrameLog = c4BootTimingCaptureActive();
      c4DiagRecordFrameTimestamp();
      if (!suppressLiveFrameLog) {
        c3PrintFrame();
      }
#else
      c3PrintFrame();
...
#if TC_TEST_MODE == 8
  c4DiagPiTaskStartUs = micros();
#endif

#if TC_TEST_MODE == 8
  c4PiSerialBefore = micros();
#endif
  Serial.println("[PiTask] start on Core1");
#if TC_TEST_MODE == 8
  c4PiSerialAfter = micros();
#endif
...
#if TC_TEST_MODE == 8
  c4DiagTcTaskStartUs = micros();
#endif

  c4TcSerialBefore[0] = micros();
  Serial.println("[TcTask] start on Core0");
  c4TcSerialAfter[0] = micros();
...
void setup() {
  ...
  Serial.println();
  Serial.println("=== ESP32-S3 TC Bridge RTOS Phase3 / Step4C C4 ===");
...
#if TC_TEST_MODE == 8
  c4DiagInterruptAttachUs = micros();
#endif
