/*
  ...
  Base:
    ESP32-S3 TC Bridge RTOS Phase3 Skeleton (user-provided mainline)

  Stage C1 change:
    - TC_TEST_MODE=5 added for Original 250201 Direction-B continuous RX diagnostics
    - p11TcRxIsr() keeps ISR minimal
    - fixed one-shot edge capture is NOT used in mode 5
    - continuous ring buffer is used in mode 5
    - provisional long-HIGH -> FALLING guard boundary candidates are reported
    - NO byte decode / NO 6-byte frame decode / NO Pi forwarding yet

  Existing Phase1.1 / 1.2 / 1.3.5-A code is retained for regression.
*/
...
//   5 = Step4C Stage C1: Original250201 continuous edge/guard monitor  <-- current
//   0 = Phase3 legacy command/response model
#define TC_TEST_MODE 5
...
// ======================================================
// Step4C Stage C1 continuous ring buffer
// ======================================================
#if TC_TEST_MODE == 5
...
static void c1ProcessEdge(uint32_t t, uint8_t lvl) {
  ...
  // Because lvl is the level AFTER this edge:
  // previousLevel==HIGH && lvl==LOW means a falling edge,
  // and dt is the duration of the HIGH period immediately before it.
  if ((c1PrevLevel == HIGH) &&
      (lvl == LOW) &&
...
static void taskTcBus(void* pv) {
  ...
  while (true) {
    ...
    // No catch-up loop. Yield after draining currently pending edges.
    vTaskDelay(pdMS_TO_TICKS(1));
  }
...
void setup() {
  ...
  Serial.println();
  Serial.println("=== ESP32-S3 TC Bridge RTOS Phase3 / Step4C C1 ===");
