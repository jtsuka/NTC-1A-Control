/*
  ...
  --------------------------------------
  Step4C Stage C4 SILENT isolation build (2026-09-01)

  Base:
    Step4C Stage C3 integrated build (2026-09-01)

  SILENT test change:
    - USB CDC Serial debug output is completely disabled in TC_TEST_MODE=8
    - SerialPi (Pi UART, 9600bps) remains enabled and unchanged
    - original 500ms pre-attach delay is retained to avoid changing startup timing
    - C1/C2/C3/C4 logic, queue depth, xQueueSend(...,0), timing and payload are unchanged
...
//
// Step4D startup-isolation test:
// USB CDC debug output is intentionally disabled completely.
// Keep TC receive/decode/queue/Pi UART behavior unchanged.
//
...
// Boot marker for Pi-side reset/reflash timestamping.
// XIAO ESP32-S3 D2 = GPIO3. Change this one line if D2 is used by your board.
static constexpr int PIN_BOOT_MARKER = D2;  // GPIO3
...
void setup() {
  ...
  // Boot marker:
  // Pi input should use a pull-down. While ESP is reset/bootloader/high-Z,
  // the line stays LOW. Firmware reaching setup() drives one clean LOW->HIGH edge.
  digitalWrite(PIN_BOOT_MARKER, LOW);
  pinMode(PIN_BOOT_MARKER, OUTPUT);
  delayMicroseconds(100);
  digitalWrite(PIN_BOOT_MARKER, HIGH);

#if C4_USB_DEBUG
  Serial.begin(115200);
#endif
  // Preserve the original pre-attach startup timing for this isolation test.
  // The delay occurs before attachInterrupt(), so it cannot build a C1 backlog.
  delay(500);
