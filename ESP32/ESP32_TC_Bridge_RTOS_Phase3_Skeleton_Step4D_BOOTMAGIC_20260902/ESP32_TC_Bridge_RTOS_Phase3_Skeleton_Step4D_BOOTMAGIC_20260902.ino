...
static void taskPiUart(void* pv) {
  ...
  // Step4D local diagnostic boot marker (ESP -> Pi UART only).
  // This is HardwareSerial(1), not USB CDC.
  // Send once before draining qTcToPi so wire order is always:
  //   ESP_BOOT_MAGIC -> normal TC telemetry.
  // Pi logger/service should already be running before ESP reset/release
  // when deterministic boot-event capture is required.
