    ...
    if (!tcMainTx.busy()) {
      ...
      if (xQueueReceive(qPiToTc, &cmd, pdMS_TO_TICKS(1)) == pdTRUE) {
        ...
        switch (cmd.data[0]) {
          ...
          case 0x10: // SAFE ON  - Stage3では未実装。ログのみ。
          case 0x11: // SAFE OFF - Stage3では未実装。ログのみ。
            Serial.print("[TcTask] SAFE command received (0x");
            Serial.print(cmd.data[0], HEX);
            Serial.println(") - not handled in Stage3 yet");
            break;
          default:
