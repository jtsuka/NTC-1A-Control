...
// ======================================================
// Recovery RESET ABORT 付帯フェイルセーフ試験専用ビルド
// ======================================================
// 本番ESP32_TC_Bridge_RTOS_Phase3_Skeleton_Stage5_fault_recovery.inoは
// 一切変更していない(このファイルはその複製に、以下のtaskTcBus()
// MODE0ループへの1-shot遅延注入だけを加えたものである)。
// origin A用TESTビルド(TEST_INJECT_TCMAINTX_OVERRUN)とは別の
// #define・別のフラグ変数を用い、注入条件をactiveRecoveryReset==true
// (recovery RESET送信中のみ)に限定する。
// TcMainTxクラス本体・poll()内部・TX_SLOT_OVERRUN_US_TCMAINTX等の
// 定数は無変更。taskPiUart()・SAFE/fault/recovery本体ロジックも
// 無変更。注入はtaskTcBus()側のみで行う。
#define TEST_INJECT_RECOVERY_OVERRUN
...
// ============================================================
// Stage5: fault / recovery RESET 用 Core0->Core1 一方向通知
// (Step3C_fault_recovery設計_最終版.md §1 準拠)
// writer=Core0(taskTcBus)、reader=Core1(taskPiUart)のみ。
// Core1側からカウンタをクリアしない(前回確認値との差分で検出する)。
// ============================================================
static std::atomic<uint32_t> gNonRecoveryAbortCount{0};   // origin A(通常command ABORTED)
static std::atomic<uint32_t> gRecoveryResetDoneCount{0};  // recovery RESET成功
static std::atomic<uint32_t> gRecoveryResetAbortCount{0}; // recovery RESET失敗
...
static void taskPiUart(void* pv) {
  ...
  // Stage5: localPending(Core1ローカル順序保持バッファ、FreeRTOS queueではない)。
  // qPiToTcがfullの場合の退避先。producer/consumerともこのtaskのみ。
  // (Step3C_fault_recovery設計_最終版.md §1・§4・§6準拠)
  ...
  // pendingCount>0の間は、新規packetをqPiToTcへ直接投入するfast pathを
  // 使わず、必ずlocalPending末尾へ追加する(追い越し禁止)。
  ...
  // Stage5: fault状態。Core1のみが所有。
  bool faultState = false;
  bool recoveryResetPending = false; // recovery RESET同時1件制限

  // Core0->Core1通知(atomic)の前回確認値。Core1からatomic自体はクリアしない。
  ...
  while (true) {
    // ------------------------------------------------------------
    // Stage5: Core0->Core1 fault/recovery通知確認。
    // 新規UART packetを受理するより必ず先に行う
    // (Step3C_fault_recovery設計_最終版.md §6の実行順序準拠)。
    // ------------------------------------------------------------
    {
    ...
    while (SerialPi.available()) {
      ...
      if (tc::checksum7(piBuf, piExpectedLen - 1) == piBuf[piExpectedLen - 1]) {
        ...
        if (faultState) {
          // fault中はRESETのみrecovery候補として例外受理する。
          ...
          // 通常時: 全command共通、localPending経由でqPiToTcへ。
          if (tryAccept(msg)) {
            debugDumpMessage("[PiTask RX]", msg);
          } else {
            // localPendingも満杯 = 最初のdrop -> fault(origin B)。
            faultState = true;
...
static void taskTcBus(void* pv) {
  ...
  // Stage5: fault latch。origin A(通常command ABORTED)発生時にtrueとなり、
  // SAFE ON/OFFでは解除できない(Step3C_fault_recovery設計_最終版.md §2・§3)。
  // recovery RESET成功時のみfalseへ戻る。
  bool localFaultLatched = false;

  // Stage5: 直前にdispatchしたRESETがrecovery文脈だったかを、
  // consumeDoneFlag()での完了判定まで保持する(同時に1件のみ有効)。
  bool activeRecoveryReset = false;

#ifdef TEST_INJECT_RECOVERY_OVERRUN
  // Recovery RESET ABORT試験専用: 1回だけ遅延を注入しoverrunを誘発する。
  // origin A用のtestOverrunInjectedとは別変数(同時に有効化しても
  // 混同しないようにするため)。TcMainTx本体・poll()内部・
  // TX_SLOT_OVERRUN_US_TCMAINTX等の定数には一切手を加えない。
  // 注入はここ(taskTcBus()側)のみ。
  bool testRecoveryOverrunInjected = false;
#endif

  while (true) {
#ifdef TEST_INJECT_RECOVERY_OVERRUN
    // recovery RESET送信中(activeRecoveryReset==true)かつbusy中の
    // 場合に限り、1回だけdelayMicroseconds()でpoll()呼び出し間隔を
    // 広げ、TcMainTx内部のoverrun判定(nextDueUs+1000us)を意図的に
    // 超えさせる。3300us slotに対して6000usの遅延で十分な余裕を
    // 持ってABORTを誘発する(origin A用と同じ遅延量)。
    if (tcMainTx.busy() && activeRecoveryReset && !testRecoveryOverrunInjected) {
      testRecoveryOverrunInjected = true;
    ...
    if (tcMainTx.consumeDoneFlag(&aborted)) {
      if (activeRecoveryReset) {
        // recovery RESETの完了(Step3C_fault_recovery設計_最終版.md §5)。
        if (aborted) {
          // fault起源に関わらず、recovery RESET自身の物理送信異常として
          // origin A相当のCore0安全状態へ強制収束させる。
        ...
        // origin A: 通常command(非recovery)のTcMainTx ABORTED。
        localFaultLatched = true;
        localSafeEnabled  = true;
    ...
    if (!tcMainTx.busy()) {
      ...
      if (xQueueReceive(qPiToTc, &cmd, pdMS_TO_TICKS(1)) == pdTRUE) {
        ...
        switch (cmd.data[0]) {
          case 0x01: { // RESET
            if (isRecoveryReset) {
              // recovery RESET: localFaultLatched/localSafeEnabledの
              // 値に関わらず必ず実行する(SAFEバイパス、fault中の唯一の例外)。
              ...
              if (tcMainTx.sendReset(len0, len1, len2, tens)) {
                ...
                // dispatch自体に失敗した場合も、recovery pendingの
                // lifecycleを必ず完結させる(Core1側の永久ロック防止)。
                // activeRecoveryResetはfalseのまま(送信を開始していないため)。
                localFaultLatched = true;
                localSafeEnabled  = true;
                gRecoveryResetAbortCount.fetch_add(1);
          ...
          case 0x11: // SAFE OFF
            if (localFaultLatched) {
              // fault latchはSAFE OFFで解除しない(今回の核心修正)。
