/*
  ...
  Stage C3 change:
    - TC_TEST_MODE=7 added for Original250201 Direction-B 6-byte frame assembly
    - Stage C1 ring/guard detector and Stage C2 byte decoder are retained
    - 5 data bytes + footer 0x7F are assembled as one Direction-B frame
    - 0x7F validates frame phase and is used to resynchronize after mid-stream start
    - lower-layer loss invalidates the current frame and waits for the next footer
    - NO Pi forwarding yet (reserved for Stage C4)
...
//   7 = Step4C Stage C3: 6-byte frame assembly + 0x7F validation/resync  <-- current
//   0 = Phase3 legacy command/response model
#define TC_TEST_MODE 7
...
// ======================================================
#if TC_TEST_MODE == 5 || TC_TEST_MODE == 6 || TC_TEST_MODE == 7
...
#if TC_TEST_MODE == 6 || TC_TEST_MODE == 7
// ------------------------------------------------------
...
#endif // TC_TEST_MODE == 6 || TC_TEST_MODE == 7

#endif // TC_TEST_MODE == 5 || TC_TEST_MODE == 6 || TC_TEST_MODE == 7
...
void setup() {
  ...
  Serial.println();
  Serial.println("=== ESP32-S3 TC Bridge RTOS Phase3 / Step4C C3 ===");
  ...
  // Stage C1/C2/C3 only observe GPIO4; they never drive GPIO4.
