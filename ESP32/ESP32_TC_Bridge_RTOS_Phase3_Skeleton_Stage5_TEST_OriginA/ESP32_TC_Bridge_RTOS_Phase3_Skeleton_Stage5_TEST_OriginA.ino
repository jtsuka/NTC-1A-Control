...
// ======================================================
// origin A(TcMainTx ABORTED)試験専用ビルド
// ======================================================
...
// 定数は無変更。注入はtaskTcBus()側のみで行う。
#define TEST_INJECT_TCMAINTX_OVERRUN
...
#ifdef TEST_INJECT_TCMAINTX_OVERRUN
  // origin A試験専用: 1回だけ遅延を注入しoverrunを誘発する。
  // TcMainTx本体・poll()内部・TX_SLOT_OVERRUN_US_TCMAINTX等の定数には
  // 一切手を加えない。注入はここ(taskTcBus()側)のみ。
  bool testOverrunInjected = false;
#endif

  while (true) {
#ifdef TEST_INJECT_TCMAINTX_OVERRUN
    // 通常command送信中(busy中)かつrecovery文脈でない場合に限り、
    // 1回だけdelayMicroseconds()でpoll()呼び出し間隔を広げ、
    // TcMainTx内部のoverrun判定(nextDueUs+1000us)を意図的に超えさせる。
    // 3300us slotに対して6000usの遅延で十分な余裕を持ってABORTを誘発する。
    if (tcMainTx.busy() && !activeRecoveryReset && !testOverrunInjected) {
      testOverrunInjected = true;
