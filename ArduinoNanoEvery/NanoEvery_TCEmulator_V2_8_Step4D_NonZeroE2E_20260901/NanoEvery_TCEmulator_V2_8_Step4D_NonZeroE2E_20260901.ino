/**
 * NanoEvery_TCEmulator_V2_8_Original250201.ino
 *
 ...
 *
 * Step4D-4 non-zero E2E TEST BUILD:
 *   ORIGINAL_250201 continuous transport is unchanged.
 *   Only emulator stub values are set to produce 23 05 01 07 03 7F
 *   from the second frame onward for Pi semantic conversion validation.
 *
 ...
 *
 * ● 送信動作の原典整理 (2026-08-31)
 *
 *   GB18030原本 FW_TC106-1_v0.0_250201.c は tx_enable を持たず、
 *   Direction-B は連続送信する。240310版も通信経路は実質同一。
 *   2025-07-28頃の tx_enable 付きSEND-gated版は試験改造と判明したため、
 *   本V2.8の正式Profileから除外する。
 *
 *   正式: TC106_ORIGINAL_250201 = 連続送信 + 250201 payload
 *   試験: TC106_TEST            = Step4B固定payloadの回帰試験
 *
 * ============================================================
...
enum FrameEndAction : uint8_t {
    FRAME_STOP = 0,      // TEST: 1frameで停止
    FRAME_CONTINUE = 1,  // ORIGINAL_250201: 次frameへ連続送信
};
...
// (極性反転版の専用複製)を、Profile(ORIGINAL_250201/TEST)をcompile-timeで
// 切り替え可能な単一エンジンへ置き換える。
//
...
// ============================================================
// 正式系はGB18030原本250201の通信仕様へ一本化する。
// 2025-07-28 Test-Gated(tx_enable)は履歴資料としてのみ保持し、
// 実装Profileからは除外する。TESTはStep4B回帰用として残す。
#define TC106_ORIGINAL_250201  1
#define TC106_TEST             2

#ifndef TC106_ACTIVE_PROFILE
#define TC106_ACTIVE_PROFILE TC106_ORIGINAL_250201
#endif

#if (TC106_ACTIVE_PROFILE != TC106_ORIGINAL_250201) && \
    (TC106_ACTIVE_PROFILE != TC106_TEST)
#error "TC106_ACTIVE_PROFILE must be TC106_ORIGINAL_250201 or TC106_TEST"
#endif
...
// Step4D-4 non-zero E2E test profile.
// Transport/timing/Profile are unchanged: ORIGINAL_250201 continuous TX.
// Expected steady-state payload from the 2nd frame onward:
//   23 05 01 07 03 7F
//   remaining_length = 23 + 5*100 + 1*10000 = 10523
//   tension_actual   = 7 + 3*10 = 37
//
// NOTE:
//   txtens is computed immediately before byte3, so the first frame already
//   carries 07 03. lastlen is computed at frame end exactly like the original
//   firmware, so byte0..2 become 23 05 01 from the second frame onward.
static uint8_t g_stubTensActual = 37;  // -> txtens[0]=7, txtens[1]=3
static int8_t  g_stubSetlen[3]  = {23, 5, 1};
static int8_t  g_stubLen1[3]    = {0, 0, 0};

// txtens計算(ORIGINAL/TEST共通の計算式。原典tx_bytecntr==3到達相当の
...
// lastlen計算(ORIGINAL/TEST共通の計算式。原典tx_bytecntr==6到達相当の
// タイミング、かつTX Policyのframe-end処理より後にCommon Engineから
// 呼ぶ。250201原典どおり毎frame計算する)。
static void payloadComputeLastlen() {
    // FW_TC106-1_v0.0_250201.c の桁借り伝播をそのまま再現する。
    int16_t tmp_1 = g_stubSetlen[1];
    int16_t tmp_2 = g_stubSetlen[2];

    int16_t v0 = (int16_t)g_stubSetlen[0] - (int16_t)g_stubLen1[0];
    if (v0 < 0) {
        v0 += 100;
        tmp_1--;
        if (tmp_1 < 0) {
    ...
    g_lastlen[0] = (int8_t)v0;

    int16_t v1 = tmp_1 - (int16_t)g_stubLen1[1];
    if (v1 < 0) {
        v1 += 100;
        tmp_2--;
    }
    g_lastlen[1] = (int8_t)v1;
    g_lastlen[2] = (int8_t)(tmp_2 - (int16_t)g_stubLen1[2]);
}
...
static const char* const TC106_PROFILE_NAME = "ORIGINAL_250201";

// GB18030原本250201: Direction-Bは停止せず連続送信する。
static FrameEndAction txPolicyOnFrameEnd() {
...
#elif TC106_ACTIVE_PROFILE == TC106_TEST

static const char* const TC106_PROFILE_NAME = "TEST";

// Step4B回帰: 固定payloadを1frame送信して停止する。
static FrameEndAction txPolicyOnFrameEnd() {
    phyWriteIdleOrZero();
    return FRAME_STOP;
}
...
// ============================================================
#if TC106_ACTIVE_PROFILE == TC106_TEST
...
#else // TC106_ORIGINAL_250201

static uint8_t payloadGetByte(uint8_t byteIndex) {
    switch (byteIndex) {
        case 0: return (uint8_t)g_lastlen[0];
        case 1: return (uint8_t)g_lastlen[1];
        case 2: return (uint8_t)g_lastlen[2];
...
static bool tcTxEnginePoll() {
    ...
    if (g_txEngSlotIndex >= 1 && g_txEngSlotIndex <= 7) {
        ...
        // false(TEST/ORIGINAL_250201): 何もしない。slot9〜15と同様、
    ...
    if (g_txEngSlotIndex == 17) {
        ...
        if (g_txEngByteIndex >= FRAME_LEN) {
            ...
            payloadComputeLastlen(); // ORIGINAL/TEST共通、Policyより後
        ...
        g_txEngByteT0Us  = g_txEngByteT0Us + (uint32_t)17 * SLOT_US;
        g_txEngSlotIndex = 1;
        // txtens計算(ORIGINAL/TEST共通。原典tx_bytecntr==3到達相当の
        // タイミング=byte3を開始する直前)。
...
// ============================================================
// 注記: 2025-07-28 Test-Gated由来処理の除外
// ============================================================
// tx_enable=0/PORT_TX=1による最終guard短縮は、原本250201には存在しない。
// V2.8では正式Profileから除外した。Common Engineのhook自体はTEST回帰との
// 構造互換のため残すが、ORIGINAL_250201/TESTともfalseを返す。
...
void loop() {
  ...
  #if TC106_ACTIVE_PROFILE == TC106_TEST
    // Step4B固定payload回帰試験。1秒ごとに1frame送信する。
    ...
    if (tcTxEngineBusy()) {
        if (tcTxEnginePoll()) {
            Serial.print('[');
            Serial.print(millis());
            Serial.println(" ms] [TEST] frame done (or aborted)");
        }
    }

  #elif TC106_ACTIVE_PROFILE == TC106_ORIGINAL_250201
    // GB18030原本250201準拠: 起動後はDirection-Bを連続送信する。
    static bool original250201Started = false;
    if (!original250201Started) {
        original250201Started = true;
        Serial.println("[ORIGINAL-250201] starting continuous TX");
        tcTxEngineStart();
    }
    if (tcTxEngineBusy()) {
        if (tcTxEnginePoll()) {
            // FRAME_CONTINUEなので通常は来ない。overrun等の異常終了のみ。
            Serial.print('[');
            Serial.print(millis());
            Serial.println(" ms] [ORIGINAL-250201] frame aborted unexpectedly");
        }
    }
  #endif
    ...
    // TC106 風の応答フレームを返す。
    //
    // 【LEGACY TEST】旧Phase1.2受信→応答ハーネス。
    // これはGB18030原本250201のDirection-B連続送信仕様ではない。
    //
    // 確認手順:
 ...
 * tx_bytecntr=6               ORIGINAL_250201では次frameへ連続継続
 *
 * ─────────────────────────────────────────────────────────────
 * [送信フレームの 0x7F の特殊波形]
 *
