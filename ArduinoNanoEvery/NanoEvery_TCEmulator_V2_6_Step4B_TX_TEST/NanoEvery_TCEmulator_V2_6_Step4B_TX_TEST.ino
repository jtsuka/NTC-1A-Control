...
// 正式PASS済み)を1行も変更せず、極性のみを反転させた派生版として
// 新規に追加する。17-step/3.3ms/7bit LSBファースト/non-blocking/
// no catch-up/overrun abort/6byte/0x7Fフッタの構造・定数(FRAME_LEN,
// SLOT_US, TX_SLOT_OVERRUN_US)は完全に共用・同一。
//
// Step4で確定した物理極性(合計1段反転)に基づく、Main/ESP32が実際に
// 見るべき論理値:
//   idle       : LOW   (原本: HIGH)
//   slot0      : LOW   (原本: HIGH)
//   bit = 0    : LOW   (原本: HIGH)
//   bit = 1    : HIGH  (原本: LOW)
//   slot 8〜16 : HIGH  (原本: LOW)
//
// フレーム先頭byte0のMain側同期成立性(Edge_checkがbyte0をどう捕捉
// するか)は本スレッドの原典再確認の結果、依然未確定・実測対象である。
// Nano側はTC106 FWの波形をそのまま忠実に再現するのみとし、この論点は
// 実TC106到着後の実測確認対象として引き継ぐ(Nano TX実装のブロッカーとはしない)。

// Step4 B方向の「idle/slot0/bit=0」状態を出力する
static void writeBusStep4B_IdleOrZero() { digitalWrite(TX_PIN, LOW); }
// Step4 B方向の「bit=1/guard」状態を出力する
static void writeBusStep4B_OneOrGuard() { digitalWrite(TX_PIN, HIGH); }

enum TxNbState4B : uint8_t {
    TXNB4B_IDLE = 0,
    TXNB4B_SENDING,
};

static TxNbState4B txNbState4B        = TXNB4B_IDLE;
static uint8_t     txNbFrame4B[FRAME_LEN];
static uint8_t     txNbFrameLen4B     = 0;
static uint8_t     txNbByteIndex4B    = 0;   // 0..txNbFrameLen4B-1
static uint8_t     txNbSlotIndex4B    = 0;   // 次に処理すべきスロット番号(1..16)
static uint32_t    txNbByteT0Us4B     = 0;   // 現バイトのslot0基準時刻(micros())
static uint32_t    txNbOverrunCount4B = 0;   // ログ用: overrun発生回数

// 現在送信中かどうか(Step4 B方向)
static bool tcTxBusyStep4B() {
    return txNbState4B != TXNB4B_IDLE;
}

// 非ブロッキング送信を開始する(Step4 B方向極性)。
// 既に送信中(IDLEでない)場合は false を返し、何もしない。
static bool tcTxStartNonBlockingStep4B(const uint8_t* frame, uint8_t len) {
    if (txNbState4B != TXNB4B_IDLE) return false;
    if (len == 0 || len > FRAME_LEN) return false;

    memcpy(txNbFrame4B, frame, len);
    txNbFrameLen4B  = len;
    txNbByteIndex4B = 0;

    // slot0: Step4 B方向のidle/slot0値(LOW)。原本はwriteBusHigh()。
    writeBusStep4B_IdleOrZero();
    txNbByteT0Us4B  = micros();
    txNbSlotIndex4B = 1;
    txNbState4B     = TXNB4B_SENDING;
    return true;
}

// 毎loop()で呼ぶ。次のスロット時刻に達していれば1スロットだけ処理する。
// フレーム送信が完了(正常終了 or overrunによるabort)した場合に true を返す。
static bool tcTxPollNonBlockingStep4B() {
    if (txNbState4B == TXNB4B_IDLE) return false;

    const uint32_t now = micros();
    const uint32_t targetUs = txNbByteT0Us4B + (uint32_t)txNbSlotIndex4B * SLOT_US;
    ...
    if ((int32_t)(now - targetUs) > (int32_t)TX_SLOT_OVERRUN_US) {
        ...
        Serial.print('[');
        Serial.print(millis());
        Serial.print(" ms] [TX-NB-4B-OVERRUN] #");
        Serial.print(txNbOverrunCount4B);
        Serial.print(" byte=");
        Serial.print(txNbByteIndex4B);
        Serial.print(" slot=");
        Serial.print(txNbSlotIndex4B);
        Serial.print(" lateBy_us=");
        Serial.println((int32_t)(now - targetUs));
        writeBusStep4B_IdleOrZero(); // Step4 B方向のidleへ復帰(原本はwriteBusHigh())
        txNbState4B = TXNB4B_IDLE;
        return true; // このフレームの送信は終了(異常終了)扱い
    }

    // ---- このスロットの処理(Step4 B方向極性) ----
    if (txNbSlotIndex4B >= 1 && txNbSlotIndex4B <= 7) {
        // slot1〜7: データ7bit、LSB first。bit=0→LOW、bit=1→HIGH(原本は逆)
        const uint8_t data = txNbFrame4B[txNbByteIndex4B];
        const bool bitIsOne = (data >> (txNbSlotIndex4B - 1)) & 0x01;
        if (bitIsOne) writeBusStep4B_OneOrGuard(); else writeBusStep4B_IdleOrZero();
    } else if (txNbSlotIndex4B == 8) {
        // slot8: footer開始。HIGH固定(原本はLOW)。slot9〜16は維持のみ。
        writeBusStep4B_OneOrGuard();
    }
    // slot9〜16: 何もしない(footer継続、タイミング消費のみ、原本と同一)

    if (txNbSlotIndex4B == 17) {
        // バイト境界: 次バイトへ、またはフレーム完了
        txNbByteIndex4B++;
        if (txNbByteIndex4B >= txNbFrameLen4B) {
            // フレーム完了 → idle(LOW、原本はHIGH)
            writeBusStep4B_IdleOrZero();
            txNbState4B = TXNB4B_IDLE;
            return true;
        }
        // 次バイトのslot0(LOW、原本はHIGH)を即座に出力
        writeBusStep4B_IdleOrZero();
        txNbByteT0Us4B  = txNbByteT0Us4B + (uint32_t)17 * SLOT_US;
        txNbSlotIndex4B = 1;
        return false; // フレームはまだ完了していない
    }
