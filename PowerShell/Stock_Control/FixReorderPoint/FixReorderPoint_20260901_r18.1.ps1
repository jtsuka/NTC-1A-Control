...
# r18.1 (2026-09-01): 重複判定を「バーコード貼付シート全体一律」から「CSVデータセット単位」へ変更。
#                    2CSV拠点(外注鉄/成果連絡表(色物))で、異なる在庫元(倉庫CSV/外注CSV)に同一品番が
#                    1件ずつ存在するcross-dataset同一品番を正常データとして許容し、同一データセット内の
#                    真の重複のみを従来どおり補正対象外とする。cross-dataset時に採用する在庫値は
#                    「バーコード貼付」シート上の物理行first hit(＝数字シートD列のVLOOKUP先頭一致と整合)。
#                    在庫の合算・後勝ち・正値優先は行わない。same-dataset重複ログのDataset表示は、
#                    実際に2件以上出現したデータセットのみ列挙する（first hitのデータセットに固定しない）。
#                    r17原本を基底とした最小差分。CSV照合本体/Get-LastRow/AutoFilter解除/ExitCode処理は
#                    変更なし。1CSV・旧RK-10互換(Skipped)は従来完全互換。
$ScriptVersion = "2026-09-01-r18.1"
$excel = $null
$workbook = $null
$barcodeSheet = $null
$startTime = Get-Date
...
function Test-BarcodePasteAgainstCsv {
    ...
    # シートの最終使用行は、フィルター状態の影響を受ける End(xlUp) を使わず、
    # 発注点補正本体と同じ UsedRange ベースの Get-LastRow で取得する。
    # 多少 UsedRange が広めでも、後段で A 列の最終非空白行を逆走査するため問題ない。
    $sheetLastRow = Get-LastRow $Sheet
...
try {
    ...
    # 人が「バーコード貼付」シートをフィルターしたまま保存しても、
    # CSV照合・発注点補正は常に全行表示の状態で実行する。
    # ShowAllData は絞り込み条件だけを解除し、オートフィルターの矢印自体は残す。
    # Updateモードでは解除状態を直ちに保存して、後続処理の成否にかかわらず次回へ残さない。
    $barcodeFilterCleared = $false
    try {
        if ([bool]$barcodeSheet.FilterMode) {
            Write-Log "INFO" "バーコード貼付シートにフィルターが適用されていたため解除しました。"
            $barcodeSheet.ShowAllData()
            $barcodeFilterCleared = $true
            if ($Mode -eq "Update") {
                $workbook.Save()
                Write-Log "INFO" "バーコード貼付シートのフィルター解除状態を保存しました。"
            }
            else {
                Write-Log "INFO" "Auditモードのため、フィルター解除状態は保存しません。"
            }
        }
    }
    catch {
        throw ("Failed to clear filter on バーコード貼付 sheet: {0}" -f $_.Exception.Message)
    }
    ...
    # =========================================================================
    # 品番→在庫数の対応表を作成する（r18.1: データセット単位の重複判定）。
    #   従来(r17)は「バーコード貼付シート全体」で同一品番を一律に重複除外していたため、
    #   2CSV拠点(外注鉄/成果連絡表(色物))で、異なる在庫元(倉庫CSV/外注CSV)に同一品番が
    #   1件ずつ存在するだけで正常データまで除外されていた。
    #   r18.1では重複判定を「CSVデータセット(ブロック)単位」に変更する。
    #     ・同一データセット内で同じ品番が複数存在      → 真の重複として補正対象外（誤補正防止）。
    #     ・異なるデータセットに同じ品番が1件ずつ存在    → 正常データ。除外しない。
    #   採用する在庫値は「バーコード貼付」シート上の物理行 first hit（最小行）とする。
    #   これは数字シートD列の VLOOKUP(...,FALSE) 先頭一致と整合させるための必須要件であり、
    #   在庫の合算・後勝ち・正値優先は行わない。
    # =========================================================================
    $inventory = New-Object "System.Collections.Generic.Dictionary[string,decimal]" ([System.StringComparer]::OrdinalIgnoreCase)

    # データセット境界の再導出（CSV照合本体は変更せず、解決済み開始行のみ再利用）。
    # 2CSV構成が有効なとき($CsvValidation2Enabled)だけ、物理行の大小でブロック境界を決める。
    #   startB = max(CsvStartRow, CsvStartRow2) 以上を下段、未満を上段とする。
    #   引数名(CSV1/CSV2)ではなく物理行順で上段/下段を決めるため、主=A4034/副=A3の設定にも追従する。
    # 1CSV / 旧RK-10互換(CsvValidation=Skipped)では全行を単一データセット扱い（従来完全互換）。
    $ds2CsvActive = [bool]$CsvValidation2Enabled
    $dsStartB = [int]::MaxValue
    if ($ds2CsvActive) {
        $dsStartA = [Math]::Min($CsvStartRow, $CsvStartRow2)
        $dsStartB = [Math]::Max($CsvStartRow, $CsvStartRow2)
        if ($dsStartB -le $dsStartA) {
            # 想定外（開始行が同値等）の場合は安全側で単一データセットへフォールバック
            $ds2CsvActive = $false
            $dsStartB = [int]::MaxValue
            Write-Log "WARN" ("DatasetBoundary fallback to single: StartA={0} StartB={1}" -f $dsStartA, $dsStartB)
        }
        else {
            Write-Log "INFO" ("DatasetBoundary Active=True StartA={0} StartB={1}" -f $dsStartA, $dsStartB)
        }
    }
    else {
        Write-Log "INFO" "DatasetBoundary Active=False (single dataset)"
    }

    # first hit（最小物理行）の在庫値と、データセット別の出現回数を収集する。
    $firstStock = New-Object "System.Collections.Generic.Dictionary[string,decimal]" ([System.StringComparer]::OrdinalIgnoreCase)
    $perDatasetCount = New-Object "System.Collections.Generic.Dictionary[string,object]" ([System.StringComparer]::OrdinalIgnoreCase)

    $barcodeLastRow = Get-LastRow $barcodeSheet
    # --- バーコード貼付シートを1行ずつ読み、品番(A列)と在庫数(C列)を収集する ---
    for ($row = 2; $row -le $barcodeLastRow; $row++) {
        ...
        try {
            ...
            # この行が属するデータセットID（A=上段 / B=下段 / S=単一）。
            # 空白品番・非数値在庫は上のcontinueで除外済みのため、
            # ここでは有効な品番＋数値在庫行だけが重複判定・first hit選択の対象になる。
            if ($ds2CsvActive) {
                if ($row -lt $dsStartB) { $ds = "A" } else { $ds = "B" }
            }
            else {
                $ds = "S"
            }
            # first hit（最小物理行）を保持する。
            if (-not $firstStock.ContainsKey($code)) {
                $firstStock.Add($code, $stock)
            }
            # データセット別の出現回数を加算する。
            if (-not $perDatasetCount.ContainsKey($code)) {
                $perDatasetCount[$code] = New-Object "System.Collections.Generic.Dictionary[string,int]" ([System.StringComparer]::Ordinal)
            }
            $dsMap = [System.Collections.Generic.Dictionary[string,int]]$perDatasetCount[$code]
    ...
    # --- データセット単位の重複後判定 ---
    #   ・いずれかのデータセット内で同一品番が2件以上 → same-dataset重複 → $inventoryに入れない。
    #     別ブロックに正常な1件があっても品番全体を除外する（誤補正防止：r17の安全側方針を維持）。
    #   ・それ以外は first hit を採用し、cross-dataset同一品番は正常データとして許容する。
    #   ・ログのDatasetは、実際に2件以上出現したデータセットのみを列挙する（first hitのデータセットとは限らないため）。
    $crossDatasetCount = 0
    foreach ($code in $firstStock.Keys) {
        $dsMap = [System.Collections.Generic.Dictionary[string,int]]$perDatasetCount[$code]
        $dupDatasets = New-Object "System.Collections.Generic.List[string]"
        foreach ($kv in $dsMap.GetEnumerator()) {
            if ($kv.Value -ge 2) {
                [void]$dupDatasets.Add($kv.Key)
            }
        }
        if ($dupDatasets.Count -gt 0) {
            Write-Log "WARN" ("Duplicate product code skipped within dataset: Code={0} Dataset={1}" -f $code, (($dupDatasets | Sort-Object) -join ","))
            continue
        }
        $inventory[$code] = $firstStock[$code]
        if ($dsMap.Count -ge 2) {
            $crossDatasetCount++
        }
    }
    Write-Log "INFO" ("CrossDatasetSameCodeCount={0}" -f $crossDatasetCount)
