...
実行例
  ...
  CorrectionEnabled
    ...
    "有"以外（例："無"）を指定すると、在庫を取得せずExcelも開かず、
    ログにCorrectionSkippedを出力して正常終了（exit 0）する。
...
try {
    ...
    if ($CorrectionEnabled -ne "有") {
        # 「発注点補正_実行」が「有」以外（例：外注鉄の「無」）の拠点は、
        # Excelを開かずに即終了する。履歴CSVへの書き込みも発生しない。
        Write-Log "INFO" (
            "CorrectionSkipped: Site={0} CorrectionEnabled={1}（発注点補正_実行が「有」以外のため、Excelを開かずスキップ）" -f
