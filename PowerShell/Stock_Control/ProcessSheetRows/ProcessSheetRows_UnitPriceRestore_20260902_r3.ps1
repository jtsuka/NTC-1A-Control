...
【2026-09-02 追加】ルール⑤拡張：復活時のG/I/J/K/O列復元
  ...
  【2026-09-02追加】バックフィルモード（-BackfillMode）：
    ...
    でなくなった」という遷移のみを検知する。このため、過去にルール⑥で
    E:O列がクリアされたが、追跡CSV導入前・何らかの理由で登録が漏れた等で
    復活処理が一度も走らないまま放置された既存行（実例：仙台シート1・
    BN0365SH。C列は正常な商品名だが、G/I/J/K/O列が数式・値ともに完全な
    空欄のまま）は、通常運用では永久に検出されない。
    -BackfillMode を指定すると、追跡CSVの登録有無に関わらず全シートを
    再スキャンし、「C列が正常（非NA）」な行のうち単価シート参照設定で
    フラグが有効な列が実際に空欄の行を検出して数式を復元する。E/F/H列は
    変更しない。日次の定型運用フラグには含めず、不具合是正のための一回限り
    のスキャンとして -Mode Audit と併用して計画を確認したのち、
    -Mode Update で反映することを想定している。
...
param(
    ...
    # 【2026-09-02追加】バックフィルモード。
    # 通常のルール⑤（廃止コード復活）は「追跡CSVに“追跡中”として登録済みの
    # 商品コードが、今回NAでなくなった」という遷移だけを検知する設計のため、
    # 過去にルール⑥でE:O列がクリアされた後、何らかの理由で復活処理が正しく
    # 走らないまま放置された既存行（例：BN0365SH。C列は正常な商品名だが、
    # G/I/J/K/O列が数式・値ともに完全な空欄のまま）は、追跡履歴が無いため
    # 通常のルール⑤では永久に検出できない。
    # このスイッチを指定すると、追跡CSVの登録有無に関わらず、全シートを
    # スキャンして「C列が正常（非NA）かつ商品コードがある行」のうち、単価
    # シート参照設定でフラグが「有」の列が実際に空欄（Test-CellEmptyForRestore）
    # であるものを一括検出し、直近正常行からの数式復元計画を立てる。
    # E列・F列・H列（発注点・発注点割れ日・欠品継続日）には一切手を加えない
    # （バックフィルはG/I/J/K/O列の数式復元のみを目的とし、発注点の値自体の
    # 妥当性判断はしないため）。通常のルール①③⑤⑥処理とは独立した処理であり、
    # 日次の定型運用には含めず、不具合発覚時の一回限りの是正スキャンとして
    # 使うことを想定している（-Mode Audit と併用してまず計画を確認し、
    # 問題なければ -Mode Update で反映する）。
    [switch]$BackfillMode
)
Set-StrictMode -Version 2.0
$ErrorActionPreference = "Stop"
# バージョン管理：改修のたびにこの値を更新する
$ScriptVersion = "2026-09-02-UnitPriceFormulaRestore-r3-Backfill"
$excel = $null
$workbook = $null
$varExcel = $null
$varWorkbook = $null
        ...
        try {
            ...
            for ($row = 2; $row -le $lastRow; $row++) {
                ...
                try {
                    ...
                    # ---------------------------------------------------------
                    # 【2026-09-02追加】バックフィルモード：
                    # ここに到達するのは「C列が正常（非NA）かつ商品コードが
                    # あり、かつ追跡CSVに“追跡中”として登録されていない行」。
                    # BN0365SHのように、過去にルール⑥でクリアされたが追跡
                    # 履歴が無い（＝通常のルール⑤では検知できない）行を
                    # 一括で救済するため、-BackfillMode 指定時のみここで
                    # G/I/J/K/O列の復元計画を立てる。
                    # E/F/H列（発注点・発注点割れ日・欠品継続日）はここでは
                    # 一切変更しない（$ENew/$FNew/$HNewはすべて$nullのまま）。
                    # continueせずに処理を継続させるため、この行が通常の
                    # 4パターン判定・ルール①③でも別途対象になる場合は、
                    # そちらの候補（E/F/H更新）とは別のオブジェクトとして
                    # $candidatesに追加され、適用フェーズで両方とも反映される
                    # （書き込み先の列が重複しないため競合しない）。
                    # ---------------------------------------------------------
                    if ($BackfillMode -and ($null -ne $unitPriceConfig)) {
                        ...
                        $backfillNeeded = @($backfillPlan | Where-Object { $_.Status -eq "Restore" })
                        if ($backfillNeeded.Count -gt 0) {
                            ...
                            $backfillCandidate = [PSCustomObject]@{
                                ...
                                Reason      = "バックフィル(既存行のG/I/J/K/O列補完,追跡履歴なし)"
                            }
