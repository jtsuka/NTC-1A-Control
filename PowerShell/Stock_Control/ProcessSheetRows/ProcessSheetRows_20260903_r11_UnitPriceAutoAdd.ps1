<#
===============================================================================
ProcessSheetRows.ps1  (r10 統合版：長期滞留・廃止コード復活・数式復元・
                      業務例外設定・ログ保持を一本化)

【2026-09-03 r10 統合】
  ベース：2026-09-02 r8 StrictBackfill-CF-NoOrderException。

  【2026-09-03 r11】仙台単価の欠落品番を0円で自動仮登録
    - 対象は SiteName=仙台 かつ UnitPriceSheet=仙台単価 のみ。
    - 数値シートJ列が #N/A で、商品コード(B)・商品名(C)・在庫数(D)が取得できる行を候補化。
    - 仙台単価 A:E の最初の空き行（3～19573）へ
      A=商品コード / B=商品名 / C=在庫数 / D=0 / E=数量×単価 の数式を登録。
    - 既に仙台単価A列に存在するコードは追加しない。
    - VLOOKUP参照範囲上限19573を超える場合はエラー停止し、範囲外へは書かない。
...
$ScriptVersion = "2026-09-03-r11-UnitPriceAutoAdd-Sendai"
$excel = $null
$workbook = $null
$varExcel = $null
$varWorkbook = $null
    ...
    try {
        ...
        foreach ($item in $Items) {
            ...
            if ($nextRow -gt $MaxLookupRow) {
                $ex = [System.Exception]::new("UnitPriceAutoAdd: 仙台単価の空き行がVLOOKUP参照範囲内にありません。上限行=$MaxLookupRow")
                $ex.Data["ExitCode"] = 10
                throw $ex
            }
...
try {
    ...
    # r11: まずは仙台だけで安全に運用確認する。単価は推測せず0円で仮登録する。
    $unitPriceAutoAddEnabled = (
        ($SiteName -eq "仙台") -and
        ($null -ne $siteConfig) -and
        ((Normalize-Code $siteConfig.UnitPriceSheet) -eq "仙台単価")
    )
    Write-Log "INFO" ("UnitPriceAutoAdd Enabled={0} TargetSheet={1}" -f $unitPriceAutoAddEnabled, $(if ($unitPriceAutoAddEnabled) { "仙台単価" } else { "" }))
    ...
    if ($unitPriceAutoAddEnabled) {
        $unitPriceMasterCodes = Load-UnitPriceMasterCodes -Workbook $workbook -SheetName "仙台単価"
        Write-Log "INFO" ("UnitPriceAutoAdd ExistingCodes={0}" -f $unitPriceMasterCodes.Count)
    }
        ...
        try {
            ...
            for ($row = 2; $row -le $lastRow; $row++) {
                ...
                try {
                    ...
                    # r11: 仙台単価にコードが存在せず、J列が#N/Aの管理行を0円仮登録候補にする。
    ...
    if ($Mode -eq "Update") {
        ...
        if ($totalUpdateCandidates -gt 0) {
            ...
            # r11: 単価表を先に補完し、その後に数値シート更新・全再計算を行う。
            $unitPriceAdded = 0
            if ($unitPriceAddCandidates.Count -gt 0) {
                $unitPriceAdded = Add-UnitPriceMasterRows -Workbook $workbook -SheetName "仙台単価" -Items $unitPriceAddCandidates -FirstDataRow 3 -MaxLookupRow 19573
                Write-Log "INFO" ("UnitPriceAutoAdd Added={0}" -f $unitPriceAdded)
            }
            foreach ($item in $candidates) {
                $targetSheet = $null
                $aCell = $null
