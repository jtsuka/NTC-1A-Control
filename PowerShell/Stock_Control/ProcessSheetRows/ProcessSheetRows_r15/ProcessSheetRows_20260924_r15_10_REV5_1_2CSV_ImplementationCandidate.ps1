...
$ScriptVersion = "2026-09-24-r15.10-REV5.1-2CSV-ImplementationCandidate"
$excel = $null
$workbook = $null
$varExcel = $null
$varWorkbook = $null
...
function Write-NewProductHistoryRows {
    ...
    try {
        ...
        foreach($item in $Items){
            ...
            if($null -ne $item.PSObject.Properties["DestinationRow"]){$sheet.Cells.Item($next,6).Value2=$item.DestinationRow}
            $sheet.Cells.Item($next,7).Value2=(Get-Date)
...
function Set-NewProductBaselineInitialized {
    ...
    try {
        ...
        $sheet.Cells.Item(1,11).Value2=(Get-Date)
