...
【2026-09-02 追加】ルール⑤拡張：復活時のG/I/J/K/O列復元
  ...
  【2026-09-02 r4 修正】Invoke-RestorePlan内のCOMオブジェクト強制解放バグ修正：
    ...
    対策：Invoke-RestorePlan は $Worksheet.Application を独自に取得・解放
    することをやめ、メインループで管理している唯一のExcel.Application
    インスタンスを -ExcelApp パラメータとして明示的に受け取り、その
    CutCopyModeプロパティを操作するに留め、Release-Comは一切呼ばない
    （寿命管理をメインループの finally 節に一元化する）ように変更した。
...
$ScriptVersion = "2026-09-02-UnitPriceFormulaRestore-r5-CutCopyModeFix"
$excel = $null
$workbook = $null
$varExcel = $null
$varWorkbook = $null
...
function Invoke-RestorePlan {
    ...
    foreach ($entry in $Plan) {
        ...
        try {
            ...
            # 【2026-09-02 r5修正】$false（PowerShellのBoolean型）をそのまま
            # CutCopyModeプロパティに代入すると、COM相互運用層で
            # "'System.Boolean' から 'Microsoft.Office.Interop.Excel.
            # XlCutCopyMode' への無効なキャストです。" という例外が発生する
            # （実運用データ・BackfillModeでの実行で発生確認済み）。
            # CutCopyModeはXlCutCopyMode列挙型（False=0）を期待するため、
            # 数値の0を明示的に代入する。
        ...
        if ($candidates.Count -gt 0) {
            ...
            foreach ($item in $candidates) {
                ...
                try {
                    ...
                    # 【2026-09-02 r4修正】$excel（メインで開いている唯一のExcel
                    # Applicationインスタンス）を -ExcelApp として明示的に渡す。
                    # Invoke-RestorePlan内部ではこれ以上COM参照の解放を行わない。
                    if ($item.RestorePlan.Count -gt 0) {
                        [void](Invoke-RestorePlan -Worksheet $targetSheet -Row $item.Row -Plan $item.RestorePlan -SheetName $item.SheetName -Code $item.Code -ExcelApp $excel)
                    }
                    Write-Log "UPDATE" (
                        "Updated Sheet={0} Row={1} Code={2} [{3}]" -f
                        $item.SheetName, $item.Row, $item.Code, $item.Reason
