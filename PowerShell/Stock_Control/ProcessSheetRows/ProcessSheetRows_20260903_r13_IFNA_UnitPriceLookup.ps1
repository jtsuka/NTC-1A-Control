...
$ScriptVersion = "2026-09-03-r13-IFNA-UnitPriceLookup-NoMasterWrite"
$excel = $null
$workbook = $null
$varExcel = $null
$varWorkbook = $null
...
try {
    ...
    if ($Mode -eq "Update") {
        ...
        if (($candidates.Count -gt 0) -or ($unitPriceFormulaFixes.Count -gt 0)) {
            ...
            Invoke-ExcelFullCalculation -Excel $excel
            $workbook.Save()
            Write-Log "INFO" ("Saved. Updated={0} UnitPriceIFNAUpdated={1} TotalChanges={2}" -f $candidates.Count, $unitPriceFormulaFixes.Count, ($candidates.Count + $unitPriceFormulaFixes.Count))
        }
        else {
        ...
        Write-Log "INFO" ("Audit completed. Workbook was not changed. UnitPriceIFNACandidates={0}" -f $unitPriceFormulaFixes.Count)
    }
