...
.EXAMPLE
    powershell.exe -NoProfile -ExecutionPolicy Bypass `
      ...
      -WorkbookPath "\\192.168.0.35\disk\企画室\塚田Work\汎用在庫シナリオ用\当日処理結果\本社商品在庫日報.xlsx" `
      -SheetName "12"
...
param(
    ...
    [string]$AllowedSourceRoot = "\\192.168.0.35\disk\企画室\塚田Work\汎用在庫シナリオ用\当日処理結果",
...
try {
    ...
    $resolvedAllowedRoot = (Resolve-Path -LiteralPath $AllowedSourceRoot).ProviderPath.TrimEnd('\')

    if (-not $resolvedWorkbookPath.StartsWith($resolvedAllowedRoot + '\', [System.StringComparison]::OrdinalIgnoreCase)) {
        throw "Safety stop: WorkbookPath must be under AllowedSourceRoot. WorkbookPath=$resolvedWorkbookPath AllowedSourceRoot=$resolvedAllowedRoot"
    }

    $WorkbookPath = $resolvedWorkbookPath
    Write-TestLog "PASS" ("SourceRootCheck PASS. AllowedSourceRoot={0}" -f $resolvedAllowedRoot)
    ...
    try {
        ...
        # CopyOriginを明示して、上側の既存行書式を継承する意図を固定する。
        [void]$rowRange.Insert(-4121, 0)
    }
    finally {
        Release-Com $rowRange
    }

    Write-TestLog "PASS" ("Rows.Insert completed at Row={0} Shift=xlShiftDown CopyOrigin=xlFormatFromLeftOrAbove" -f $insertRow)
