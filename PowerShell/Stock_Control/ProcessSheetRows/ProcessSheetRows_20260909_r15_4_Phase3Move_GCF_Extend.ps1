...
$ScriptVersion = "2026-09-09-r15.4-Phase3Move-GCF-Extend"
$excel = $null
$workbook = $null
$varExcel = $null
$varWorkbook = $null
    ...
    try {
        ...
        foreach ($item in $Candidates) {
            ...
            try {
                ...
                $cells[3].Value2 = ""
                $cells[4].Value2 = $Today.ToString("yyyy/MM/dd")
...
function Invoke-Phase3Move {
    ...
    try {
        ...
        [void](Invoke-RestorePlan -Worksheet $sheet -Row $insertRow -Plan $plan -SheetName $Candidate.DestinationSheet -Code $Candidate.Code)
        ...
        try {
            ...
            if (-not $aIsValid -or
                ...
                $dText -match '^\s*#N/A\s*$') {
                throw "Phase3 runtime validation FAIL. Sheet=$($Candidate.DestinationSheet) Row=$insertRow Code=$($Candidate.Code) A=$aText ExpectedA=$managementNumber C.Text=$cText D.Text=$dText"
            }

            Write-Log "PASS" (
                "Phase3MoveValidated Sheet={0} Row={1} Code={2} A={3} C.Text={4} D.Text={5} GFormula={6} IFormula={7}" -f
                $Candidate.DestinationSheet, $insertRow, $Candidate.Code, $aNumber, $cText, $dText,
                [bool]$checkG.HasFormula, [bool]$checkI.HasFormula
            )
        }
        finally {
            Release-Com $checkI; Release-Com $checkG; Release-Com $checkD
