...
目的
  ...
  グラフ範囲から除外する単体テスト用スクリプト。

重要
  - Excel COM の Series.Formula は一切書き換えない。
  - xlsx 内部の chart XML の参照式だけを直接置換する。
  - 日付判定も xlsx 内部の worksheet XML の保存値を使用する。
  - 対象外の ZIP エントリ内容が変化した場合は失敗として元ファイルを置換しない。
  - Update 前に必ずバックアップを作成する。
  - 2回目実行で変更なしになること（冪等性）を確認できる設計。
...
try {
    ...
    if ($fileName -ne $ExpectedFileName) {
        throw "Unexpected workbook file name: $fileName . Expected: $ExpectedFileName"
    }
