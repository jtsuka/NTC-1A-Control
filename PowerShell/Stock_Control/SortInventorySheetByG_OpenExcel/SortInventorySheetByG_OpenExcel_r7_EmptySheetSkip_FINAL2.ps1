...
<#
.SYNOPSIS
  RK-10 が開いている在庫帳の数字シートをG列で明示ソートし、A列の管理番号を再採番する。WorkbookName はファイル名/フルパス双方に対応。
...
.DESCRIPTION
  ...
  想定フロー:
    RK-10でExcelを開く
      -> シートを選択
      -> 本スクリプトを Update で呼ぶ
      -> 本スクリプト内でG列ソート＋A列管理番号再採番
      -> RK-10は次シートへ

.VERSION
  ScriptVersion = 2026-09-10-r7-OpenExcel-GSort-RenumberA-EmptySheetSkip

.CHANGELOG
  2026-09-10 r7:
    - 商品コード列(B列)にデータ行が1件もない数字シートをERRORではなく正常SKIPとする。
    - Get-LastDataRowの判定方式は変更しない。B列基準を維持する。
    - UsedRangeや他列へフォールバックしない。テンプレート数式のみ残る空シートを誤って処理しないため。
    - Audit/Updateとも空シートは変更せず ExitCode=0。

.TEST TARGET
  外注鉄在庫帳 / Sheet=17
  期待: B2=見出しのみ、B3以降に商品コード0件の場合、
        NoDataRows / Sort skipped をINFO出力し、ERRORなし・変更なしで正常終了する。

  回帰確認: 外注鉄在庫帳 / Sheet=16,18
  期待: 従来どおりB列最終行を取得し、G列降順ソート＋A列再採番を行う。
...
function Write-Log {
    ...
    # RK-10 V4.0:
    #   INFO / WARN / AUDIT / UPDATE -> 標準出力 (stdout)
    #   ERROR                        -> 標準エラー (stderr)
    # RK-10側で ExitCode <> 0 の場合に標準エラーを取得してログへ残せるようにする。
    if ($Level -eq "ERROR") {
...
function Get-TargetWorkbook {
    ...
    # RK-10 からは「ファイル名だけ」でなくフルパスが渡る場合がある。
    # Excel COM の Workbook.Name は basename のみなので、まず basename を抽出する。
    $targetBookName = $Name
    try {
        if (-not [string]::IsNullOrWhiteSpace($Name)) {
            $candidate = [System.IO.Path]::GetFileName($Name)
            if (-not [string]::IsNullOrWhiteSpace($candidate)) {
                $targetBookName = $candidate
            }
        }
    }
    catch {
        # Path.GetFileName が失敗しても、元の文字列で照合を継続する。
        $targetBookName = $Name
    }

    Write-Log INFO "Workbook lookup requested='$Name' normalized='$targetBookName'"

    $found = $null

    foreach ($wb in @($ExcelApp.Workbooks)) {
        try {
            if ([string]::Equals([string]$wb.Name, $targetBookName, [StringComparison]::OrdinalIgnoreCase)) {
                $found = $wb
                break
            }
        }
    ...
    if ($null -eq $found) {
        ...
        throw "対象ブック '$Name' (normalized='$targetBookName') が開かれていません。OpenWorkbooks=[$($openNames -join ', ')]"
    }
...
function Set-ManagementNumbers {
    ...
    # 正式仕様:
    #   G=1 だけでなく G=0 も採番対象。
    #   G が数値(0/1)である行すべてに A=1,2,3... を振る。
    #   G が空欄になった最初の行以降は A を空欄にする。
    #
    # G降順ソート後は通常、
    #   1 ... 1 / 0 ... 0 / 空欄 ... 空欄
    # となることを前提とする。
    #
    # 安全策:
    #   いったん G 空欄を検出した後に再び数値0/1が出た場合は異常終了する。
    ...
    for ($r = $StartRow; $r -le $EndRow; $r++) {
        ...
        try {
            ...
            $isBlank = ($null -eq $gValue -or [string]::IsNullOrWhiteSpace([string]$gValue))
            ...
            if ($isZeroOrOne) {
                if ($seenBlank) {
                    throw "G列の空欄以降に数値0/1が存在します。Row=$r Value=$gValue。A列採番を中止します。"
                }

                $numberedCount++
                $aCell.Value2 = $numberedCount
            }
            else {
                throw "G列に想定外の値があります。Row=$r Value=$gValue。A列採番を中止します。"
            }
        }
        finally {
...
try {
    ...
    if ($lastRow -lt $DataStartRow) {
        # r7: B列の商品コードデータが0件の数字シートは異常ではない。
        # UsedRangeや他列へフォールバックすると、テンプレート数式だけが残る
        # 空シートを実データありと誤判定する可能性があるため、B列基準は維持する。
        Write-Log INFO "NoDataRows Sheet=$SheetName Column=$LastRowColumn DataStartRow=$DataStartRow. Sort/ManagementNumbering skipped."
        Write-Log INFO "Completed. No data rows; worksheet was not changed."
        exit 0
    }
    ...
    if ($Mode -eq "Audit") {
        Write-Log AUDIT "ExpectedNumberedCountAfterSort=$($beforeSummary.One + $beforeSummary.Zero) (G=1 or 0 rows)"
    ...
    Write-Log UPDATE "ManagementNumbering Sheet=$SheetName NumberedRows=$managedCount A=$DataStartRow..$($DataStartRow + $managedCount - 1) Numbers=1..$managedCount"
    Write-Log UPDATE "ManagementNumbering BlankGRows=$($lastRow - $DataStartRow + 1 - $managedCount) A cleared."
    ...
    if ($managedCount -ne ($afterSummary.One + $afterSummary.Zero)) {
        throw "A列採番件数とG=0/1件数が一致しません。Numbered=$managedCount G1=$($afterSummary.One) G0=$($afterSummary.Zero)"
    }
