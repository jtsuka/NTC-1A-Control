<#
.SYNOPSIS
    船舶値付用紙 印刷 - Excel非依存版 (PoC / v0.10.2)
...
.NOTES
    ...
    以下、2026/08/11時点で未実装/要検証の項目:
      ...
      - 原価未入力/売価未入力の警告表示 … 未実装。元VBAには存在するが、
        書込み先セル「型式」がlabel_layout_full.jsonのfield_mappingに見当たらず、
        この帳票への表示要否・表示位置ともに現場確認待ち。
      - AS12の本来の警告表示 … 未実装。現状は $override.Remove("AS12") で
        生データの誤表示のみ防いでいる状態(v0.4)。元VBAの判定条件・表示要否は現場確認待ち。
