...
function Apply-DerivedFields($override, $row, $fields, $rows) {
    ...
    # v0.10.8 / VBA原典: AV5 一式金額複合表示。
    # source=185 の生値は使用せず常にクリアし、先頭明細の請求先一式金額(183)>0をゲートに、
    # 経由先一式金額(184)・受注一式金額(186)を任意追記する。複数明細時も先頭明細を採用。
    $override.Remove("AV5") | Out-Null
    $av5Row = $rows[0]
    $billingPackage = 0.0; $routePackage = 0.0; $orderPackage = 0.0
    [void][double]::TryParse([string]$av5Row."183", [ref]$billingPackage)
    [void][double]::TryParse([string]$av5Row."184", [ref]$routePackage)
    [void][double]::TryParse([string]$av5Row."186", [ref]$orderPackage)
    if ($billingPackage -gt 0) {
        $av5 = "【一式金額 / 請求 : ￥{0:N0}" -f $billingPackage
        if ($routePackage -gt 0) { $av5 += " / 経由 : ￥{0:N0}" -f $routePackage }
        if ($orderPackage -gt 0) { $av5 += "/ 受注 : ￥{0:N0}" -f $orderPackage }
        $av5 += "】"
        $override["AV5"] = $av5
    }
