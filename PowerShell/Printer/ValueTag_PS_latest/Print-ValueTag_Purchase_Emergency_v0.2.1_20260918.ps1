...
# ============================================================
# EMERGENCY PURCHASE BUILD v0.2.1 2026-09-18
# Base: Print-ValueTag_Purchase_Emergency_v0.2_20260918.ps1
# Purpose: 購買ゼロ円値付け用紙(BT_NO=3)の緊急代替・第二波対応
# 注意: 正式REV.2共通化前の緊急分岐版。船舶v0.10.6は変更しない。
#
# v0.2.1 最小修正（2026-09-18 OCR/PDF差分確認）
#   - HEAD A7「標準表示」はDL列192「商談状況」の生値を直接表示しない。
#   - 商談状況=6 のときだけ「標準」、=1 のときだけ「後報」を表示。
#   - それ以外（0/空欄/その他）はA7を空欄とする。
#   - 実PDFで確認した T266091720 / T268083190 の不要な「0」表示を解消するための局所修正。
#   - 抽出条件、81手配、85ページ、物理ページング、その他帳票ロジックは変更しない。
#
# v0.2 検証済み修正（2026-09-18 実CSV/VBA実PDF/OCR再照合）
#   1) MakeData行フィルタの原典追従
#      - OUT_OF_STOCK=1: Z_DIC 6在庫コードを得意先番号から除外
#      - OUT_OF_PERSON=1: Excel数値化後の得意先番号が6桁以下を除外
#      - START_DATE=(Year(Date)-3)&"1216": 最終出荷日が開始日より古い行を除外
#      2026-09-18実データでは、この追加後のVBA印刷対象81手配とPS候補81手配が完全一致。
#   2) 11明細以上のページングを、単純な10件チャンクからVBA/Excel物理改ページ方式へ修正
#      - VBAの手動改ページ: 10明細ごと
#      - PrintTitleRows=$1:$3 を継続ページで再現
#      - 固定Zoom: DocuWorks=56%、その他=59%
#      - DocuWorks実PDFで確認した物理境界（64テンプレート行/頁）を基準に、BODYを行途中でも分割。
#        T26795013A/B(各11明細)は VBA同様 3ページ（1/3,2/3,3/3）になる計画。
#      ※12/15明細等は次回実データで追加回帰確認する。
#   3) v0.1は保存し、上書きしない。
# ============================================================
...
if ($CsvPath) {
    ...
    # ============================================================
    # 2026-09-18 緊急購買版
    # 元VBA BT_NO=3 の購買「ゼロ円値付け用紙」抽出条件へ変更。
    # 帳票Engineは v0.10.6 を極力そのまま流用し、抽出条件だけを最小変更する。
    # ============================================================

    # 工場番号(列1)="FAC11"のみ残す
    $validRows = $allRows | Where-Object { ([string]$_."1").Trim() -eq "FAC11" }

    # 購買(BT_NO=3): 受注番号先頭 T/E/L/H のみ
    $beforePrefix = @($validRows).Count
    $validRows = $validRows | Where-Object {
        $orderNo = ([string]$_."2").Trim()
        $orderNo.Length -gt 0 -and @("T","E","L","H") -contains $orderNo.Substring(0,1)
    }
    Write-Host "診断: 購買(T/E/L/H始まり)絞込み: $beforePrefix 件 → $(@($validRows).Count) 件"

    # ------------------------------------------------------------
    # v0.2 / VBA MakeData 共通行フィルタ（BT_NO=3では強制的に有効）
    # ------------------------------------------------------------
    # ExcelでCSVを開いた時の数値セル相当へ寄せる。
    # 例: "07000014" -> "7000014", "00203007" -> "203007"。
    # VBA原典は Z_DIC.exists(Val(得意先番号)) と Len(得意先番号) を使うため、
    # PowerShellでCSV文字列をそのまま比較すると先頭0の有無で結果がずれる。
    function ConvertTo-VbaNumericCellText([string]$Value) {
        $t = if ($null -eq $Value) { "" } else { $Value.Trim() }
        if ($t -match '^[+-]?\d+(?:\.0+)?$') {
            $n = [long]0
            $integerText = ($t -replace '\.0+$', '')
            if ([long]::TryParse($integerText, [ref]$n)) { return [string]$n }
        }
        return $t
    }

    # OUT_OF_STOCK=1 / Z_DIC。settingシート原典の6在庫コード。
    $purchaseStockCodes = @(
        "1000012",   # 札幌在庫
        "3000010",   # 仙台在庫
        "5000017",   # 東京在庫
    ...
    $beforeStock = @($validRows).Count
    $validRows = $validRows | Where-Object {
        $customer = ConvertTo-VbaNumericCellText ([string]$_.'5')
        -not ($purchaseStockCodes -contains $customer)
    }
    Write-Host "診断: 購買 在庫抜き(OUT_OF_STOCK/Z_DIC): $beforeStock 件 → $(@($validRows).Count) 件"

    # OUT_OF_PERSON=1。VBA: Len(.Cells(...得意先番号...)) <= 6 を除外。
    # Excelで数値化された後の見た目（先頭0除去後）でLenを判定する。
    $beforePerson = @($validRows).Count
    $validRows = $validRows | Where-Object {
        $customer = ConvertTo-VbaNumericCellText ([string]$_.'5')
        $customer.Length -gt 6
    }
    Write-Host "診断: 購買 担当手配抜き(OUT_OF_PERSON/Len>6): $beforePerson 件 → $(@($validRows).Count) 件"

    # BT_NO<>1では START_DATE = Year(Date)-3 & "1216" が強制設定される。
    # VBA: If START_DATE > 最終出荷日 Then GoTo DEL_ROW
    $purchaseStartDate = [long]("{0}1216" -f ((Get-Date).Year - 3))
    $beforeStartDate = @($validRows).Count
    $validRows = $validRows | Where-Object {
        $lastShipText = ConvertTo-VbaNumericCellText ([string]$_.'9')
        $lastShip = [long]0
        if (-not [long]::TryParse($lastShipText, [ref]$lastShip)) { $lastShip = 0 }
        $lastShip -ge $purchaseStartDate
    }
    Write-Host "診断: 購買 最終出荷日下限($purchaseStartDate): $beforeStartDate 件 → $(@($validRows).Count) 件"

    # 購買(BT_NO=3): 送料無料の運賃行を除外
    # 元VBA条件: 品目番号=99999900 かつ 特記事項1/2のいずれかに「送料無料」を含む
    $beforeFreight = @($validRows).Count
    $validRows = $validRows | Where-Object {
        $itemNo = ([string]$_."10").Trim()
        $note1  = ([string]$_."28").Trim()
        $note2  = ([string]$_."29").Trim()
        -not ($itemNo -eq "99999900" -and ($note1 -like "*送料無料*" -or $note2 -like "*送料無料*"))
    }
    Write-Host "診断: 送料無料運賃行除外: $beforeFreight 件 → $(@($validRows).Count) 件"

    Write-Host "診断: CSV総行数=$($allRows.Count)  購買MakeData行フィルタ後=$(@($validRows).Count)"
    ...
    # 購買(BT_NO=3)は行単位ではなく手配単位で FLAG_DIC 相当を判定する。
    # kou=1   : 手配内に 品目番号 Like "KOU*" かつ KOU000以外 が1件以上
    # genka=1 : 手配内に 品目番号<>COMENT かつ 仮単価区分=仮(=1)
    #           かつ 営業原価_単価=0 が1件以上
    # 印刷対象 = kou=1 AND genka=1
    $beforePurchaseFlags = @($grouped).Count
    $grouped = $grouped | Where-Object {
        $hasKou = $false
        $hasGenka = $false

        foreach ($r in $_.Group) {
            $itemNo = ([string]$r."10").Trim()

            if ($itemNo -like "KOU*" -and $itemNo -ne "KOU000") {
                $hasKou = $true
            }

            $kariKubun = ([string]$r."25").Trim()
            $genkaTanka = 0.0
            [void][double]::TryParse(([string]$r."208").Trim(), [ref]$genkaTanka)

            if ($itemNo -ne "COMENT" -and $kariKubun -eq "1" -and $genkaTanka -eq 0) {
                $hasGenka = $true
            }

            if ($hasKou -and $hasGenka) { break }
        }

        $hasKou -and $hasGenka
    }
    Write-Host "診断: 購買 kou AND genka 手配絞込み: $beforePurchaseFlags 件 → $(@($grouped).Count) 件"
...
function Apply-DerivedFields($override, $row, $fields, $rows) {
    ...
    # v0.2.1: field_mapping転記で入った生値(例: "0")を必ず消してから表示用文字列だけを設定する。
    #          1/6以外はVBA実出力どおり空欄。
    ...
    # 経由先１_一式金額(source=185) : AQ4/AW4と同じパターンで、値が"0"のとき
    # 生値がそのまま印字されてしまう(2026/08/10データ 28件中9件で再現確認)。
    # 元VBAは空欄のため、値が"0"のときは非表示にする。
    foreach ($f in $fields) {
        if ($f.source_field_id -eq 185) {
            if ($override.ContainsKey($f.target_cell) -and $override[$f.target_cell] -eq "0") {
...
function Apply-BodyDerivedFields($override, $row) {
    ...
    # 緊急購買版: 以下は元VBA BT_NO=2(船舶)専用表示のため無効化する。
    # BJ12の原価未入力/売価未入力警告は直前の共通処理結果をそのまま保持する。
    $override.Remove("AY12") | Out-Null
    $override.Remove("BC12") | Out-Null
    # setting転記由来のBJ12生値が残らないよう、警告が設定されていない場合だけ除去する。
    if ($override.ContainsKey("BJ12")) {
        $bj12 = [string]$override["BJ12"]
        if ($bj12 -ne "△▲△原価未入力▲△▲" -and $bj12 -ne "△▲△売価未入力▲△▲") {
            $override.Remove("BJ12") | Out-Null
        }
    }
...
# v0.2: 元VBA PrintableShapes() のページングを「10明細チャンク」ではなく、
# Excelの手動改ページ＋物理改ページとして再現する。
#   - row_count Mod 10 = 0 で次行に手動PageBreak
#   - row_count > 10 の手配は固定Zoom（DocuWorks=56%、その他=59%）
#   - PrintTitleRows="$1:$3" により、2ページ目以降はテンプレート1～3行を繰り返す
# 2026-09-18 VBA実PDFでは、DocuWorks 56%時に最初の物理ページが
# テンプレート64行相当で切れ、10明細目のBODY(6行)が2行+4行へ分割された。
$VBA_MANUAL_BREAK_ITEMS = 10
$VBA_BODY_TEMPLATE_START_ROW = $blocks.body.StartRow   # 9
$VBA_BODY_TEMPLATE_END_ROW   = $blocks.body.EndRow     # 14
$VBA_BODY_TEMPLATE_ROWS      = $VBA_BODY_TEMPLATE_END_ROW - $VBA_BODY_TEMPLATE_START_ROW + 1
$VBA_FULL_HEADER_ROWS        = 8   # top(1) + head(2～8)
$VBA_PRINT_TITLE_ROWS        = 3   # PrintTitleRows=$1:$3
$VBA_DOCUWORKS_ROW_CAPACITY  = 64  # 2026-09-18 VBA実PDFから実証
$VBA_NORMAL_ROW_CAPACITY     = 60  # 同一物理領域を59%へ換算: floor(64*56/59)

# -PrinterName未指定時はOSデフォルトプリンター名を取得し、ページ計画にも反映する。
$effectivePrinterNameForPaging = $PrinterName
if (-not $effectivePrinterNameForPaging) {
    try {
        $tmpPrinterSettings = New-Object System.Drawing.Printing.PrinterSettings
        $effectivePrinterNameForPaging = $tmpPrinterSettings.PrinterName
    } catch {
        $effectivePrinterNameForPaging = ""
    }
}
$VBA_FIXED_ROW_CAPACITY = if ($effectivePrinterNameForPaging -eq "DocuWorks Printer") { $VBA_DOCUWORKS_ROW_CAPACITY } else { $VBA_NORMAL_ROW_CAPACITY }
Write-Host "診断: VBA固定Zoom物理ページ計画: printer='$effectivePrinterNameForPaging' rowCapacity=$VBA_FIXED_ROW_CAPACITY"

# 1手配の11明細以上をVBA物理ページへ分解する。BODYは途中行でも分割可能。
# BodyParts要素: @{ Data=<BODY override>; StartRow=9..14; EndRow=9..14 }
function New-VbaFixedPhysicalPages {
    param(
        [string]$OrderNo,
        $TopHeadOverride,
        [array]$BodyOverrides,
    ...
    if ($RowCapacity -le $VBA_FULL_HEADER_ROWS) { throw "VBA固定Zoomページ容量が不正です: $RowCapacity" }

    $pages = @()
    $itemIndex = 0

    while ($itemIndex -lt $BodyOverrides.Count) {
        # VBAの手動改ページ単位: 最大10明細。次の10明細は必ず新しい物理ページから開始する。
        $segmentCount = [Math]::Min($VBA_MANUAL_BREAK_ITEMS, $BodyOverrides.Count - $itemIndex)
        $segmentItemOffset = 0
        $currentBodyRow = $VBA_BODY_TEMPLATE_START_ROW

        while ($segmentItemOffset -lt $segmentCount) {
            $isFirstPhysicalPage = ($pages.Count -eq 0)
            $headerRows = if ($isFirstPhysicalPage) { $VBA_FULL_HEADER_ROWS } else { $VBA_PRINT_TITLE_ROWS }
            $remainingRows = $RowCapacity - $headerRows
            $pageParts = @()

            while ($remainingRows -gt 0 -and $segmentItemOffset -lt $segmentCount) {
                $bodyData = $BodyOverrides[$itemIndex + $segmentItemOffset]
                $rowsLeftInBody = $VBA_BODY_TEMPLATE_END_ROW - $currentBodyRow + 1
                $takeRows = [Math]::Min($remainingRows, $rowsLeftInBody)
                $partEndRow = $currentBodyRow + $takeRows - 1

                $pageParts += , @{
                    Data     = $bodyData
                    StartRow = $currentBodyRow
                    EndRow   = $partEndRow
                }

                $remainingRows -= $takeRows
                if ($partEndRow -ge $VBA_BODY_TEMPLATE_END_ROW) {
                    $segmentItemOffset++
                    $currentBodyRow = $VBA_BODY_TEMPLATE_START_ROW
                } else {
                    # 同一BODYの残り行は次の物理ページへ。
                    $currentBodyRow = $partEndRow + 1
                }
            }

            $pages += , @{
                OrderNo          = $OrderNo
                TopHead          = $null  # 総ページ数確定後にページ別Cloneを設定
                BodyRows         = @()
                BodyParts        = $pageParts
                IsContinuation   = (-not $isFirstPhysicalPage)
                FixedScale       = $true
                PageInOrder      = 0
                TotalPagesInOrder= 0
            }
        ...
        $itemIndex += $segmentCount
    }

    # Excel CenterHeader=&P/&N 相当。各ページに独立したTopHeadを持たせる。
    $totalPages = $pages.Count
    for ($pi = 0; $pi -lt $totalPages; $pi++) {
        $pageHead = $TopHeadOverride.Clone()
        $pageHead["AH1"] = "{0}/{1}" -f ($pi + 1), $totalPages
        $pages[$pi].TopHead = $pageHead
        $pages[$pi].PageInOrder = $pi + 1
        $pages[$pi].TotalPagesInOrder = $totalPages
    }
...
# $labels の各要素は物理印刷1ページに対応。
# 10明細以下: BodyRows（従来） / 11明細以上: BodyParts（BODY途中行分割あり）を使用。
$labels = @()
...
if ($orderGroups.Count -gt 0) {
    foreach ($rows in $orderGroups) {
        ...
        if ($bodyOverrides.Count -le $VBA_MANUAL_BREAK_ITEMS) {
            # 10品目以下: VBAはZoom=False / FitToPagesWide=1 / FitToPagesTall=1。1手配=1ページ。
            $labels += , @{
                OrderNo = $orderNo; TopHead = $topHeadOverride; BodyRows = $bodyOverrides; BodyParts = @()
                IsContinuation = $false; FixedScale = $false; PageInOrder = 1; TotalPagesInOrder = 1
            }
        }
        else {
            # 11品目以上: 10明細ごとの手動PageBreakと、固定ZoomによるExcel物理改ページを再現。
            # BODYがページ境界を跨ぐ場合は、テンプレート行単位でBodyPartsへ分割する。
            $orderPages = @(New-VbaFixedPhysicalPages -OrderNo $orderNo -TopHeadOverride $topHeadOverride -BodyOverrides $bodyOverrides -RowCapacity $VBA_FIXED_ROW_CAPACITY)
            foreach ($pageDef in $orderPages) { $labels += , $pageDef }
        }
    }
    ...
    if ($ExpectedDumpDir -and $labels.Count -gt 0) {
        ...
        for ($di = 0; $di -lt $labels.Count; $di++) {
            ...
            $bodyNamed = @()
            if ($labels[$di].BodyParts -and @($labels[$di].BodyParts).Count -gt 0) {
                foreach ($part in $labels[$di].BodyParts) {
                    $rowNamed = [ordered]@{}
                    foreach ($kv in ($part.Data.GetEnumerator() | Sort-Object Name)) {
                        if ($kv.Name -like "__*") { continue }
                        $rowMatch = [regex]::Match($kv.Name, '\d+$')
                        if ($rowMatch.Success) {
                            $cellRow = [int]$rowMatch.Value
                            if ($cellRow -lt [int]$part.StartRow -or $cellRow -gt [int]$part.EndRow) { continue }
                        }
                        $name = if ($cellToLabel.ContainsKey($kv.Name)) { $cellToLabel[$kv.Name] } else { $kv.Name }
                        $rowNamed["$name ($($kv.Name))"] = $kv.Value
                    }
                    $rowNamed["__VBA_BODY_SLICE"] = "{0}-{1}" -f $part.StartRow, $part.EndRow
                    $bodyNamed += , $rowNamed
                }
            }
            else {
...
elseif (-not $CsvPath) {
    ...
    $labels += , @{ OrderNo = "SAMPLE"; TopHead = $sampleTopHead; BodyRows = $sampleBodyRows; BodyParts = @(); IsContinuation = $false; FixedScale = $false; PageInOrder = 1; TotalPagesInOrder = 1 }
}
...
function Draw-OneLabel {
    ...
    if ($Label.BodyParts -and @($Label.BodyParts).Count -gt 0) {
        foreach ($part in $Label.BodyParts) {
            $sliceBlock = @{ StartRow = [int]$part.StartRow; EndRow = [int]$part.EndRow }
            Draw-Block -Graphics $Graphics -Block $sliceBlock -YOffset $yCursor -DataOverride $part.Data -FontCache $fontCache -OffsetX $OffsetX -OffsetY $OffsetY -PenHair $penHair -PenThin $penThin -PageNo $PageNo -OrderNo $OrderNo
            $yCursor += (YOfRow ($sliceBlock.EndRow + 1)) - (YOfRow $sliceBlock.StartRow)
        }
    }
    else {
...
function Estimate-LabelHeight($Label, [bool]$IsContinuation = $false) {
    ...
    if ($Label.BodyParts -and @($Label.BodyParts).Count -gt 0) {
        foreach ($part in $Label.BodyParts) {
            $h += (YOfRow ([int]$part.EndRow + 1)) - (YOfRow ([int]$part.StartRow))
        }
    }
    else {
...
Write-Host "診断: 出力対象の手配件数 = $($orderGroups.Count)  / 印刷ページ数 = $($labels.Count)$(if ($splitCount -gt 0) { "(VBA物理改ページの継続ページ $splitCount 枚を含む)" })"
...
if ($PreviewOnly) {
    ...
    try {
        ...
        # 10品目以下は1手配=1ページで自動Fit。
        # 11品目以上はv0.2で事前にVBAの「10明細ごとの手動改ページ＋固定Zoom物理改ページ」へ分解済み。
        # BodyPartsにより、BODYブロックの途中行でページ境界を跨ぐケースも再現する。
        $script:pageIndex = 0
        $printDoc.add_PrintPage({
            ...
            $pageKind = if ($lbl.FixedScale) { if ($lbl.IsContinuation) { "VBA物理継続" } else { "VBA固定Zoom先頭" } } else { "通常Fit" }
