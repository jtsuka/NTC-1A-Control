...
//set GCP_ON
#pragma config JTAGEN = OFF       // JTAG 柍岠
#pragma config FWDTEN = OFF       // WDT 柍岠
#pragma config ICS = PGx2         // 僺儞愝掕
#pragma config FCKSM = CSDCMD     // 僋儘僢僋僗僀僢僠柍岠丄WDT柍岠
#pragma config POSCMOD = NONE     // 奜晹敪怳婍側偟
...
*************************************************************/
//const unsigned int StateLoTable[8] = {0x0000, 0x0210, 0x2004, 0x0204, 0x0801, 0x0810, 0x2001, 0x2A00};//Version2.0,硬?变竵E男?
//const unsigned int StateLoTable[8] = {0x0000, 0x0021, 0x0018, 0x0009, 0x0006, 0x0024, 0x0012, 0x0000};//电?反转，眮E-6改为6-1，此段取?了底端PWM。
//const unsigned int StateLoTable[8] = {0x0000, 0x2001, 0x0810, 0x0801, 0x0204, 0x2004, 0x0210, 0x2A00};//电?反转，眮E-6改为6-1，底端PWM
//const unsigned int StateLoTable[8] = {0x0000, 0x0120, 0x1008, 0x0108, 0x0402, 0x0420, 0x1002, 0x0000};//高段PWM

const unsigned int StateLoTable[8] = {0x0000, 0x8400, 0x1800, 0x9000, 0x6000, 0x2400, 0x4800, 0x0000};//电?换?眮E
...
volatile unsigned char setlen[3];		//设定的长度（对藖E?行?法实现长度控制?
volatile signed char len_1[3];		//剩余的长度,计算发?负?
volatile signed char lastlen[3];		//长度（已经?洹的长度
...
volatile unsigned int tens_actual;		//存放张力平?值
//volatile unsigned int tens_sample[16];	//A/D结?存放的变量
volatile unsigned int tens_sum;			//存放16次采集的AD值的总和
volatile unsigned int tens_zero;		    //存放零点张力值
volatile unsigned int tens_temp;
volatile signed int tens_deviation;			//???祿
volatile signed int last_deviation;		//前次?瞾E
volatile signed int prev_deviation;		//上次?瞾E
...
volatile unsigned int rx_timer;		//接收时间的计?苼E
...
volatile unsigned char rx_buffer[6];			//接收?据莵E
volatile unsigned int tx_timer;			//发送时间的计?苼E
...
volatile unsigned int stop_delay;			//电?停止时紒E
volatile unsigned int cutter_timer;		//?刀复位计?苼E

volatile unsigned int check_timer;			//端口查询时间计?苼E
volatile signed int pulse_cntr;				//丝长计?,在?法运算时需要负?

volatile unsigned int display_timer;				//LED闪烁延迟计?
volatile unsigned int ledcntr;				//LED闪烁延迟计?
volatile unsigned int HallValue;
volatile unsigned int eeprom_address;	//保存在eeprom中的??
volatile unsigned char zero_delay;
//?试
volatile signed int tens_addt[data_total];//记录不同时间段的补偿值
//2014.8.24改为190??据,200??据为极限,再?加变量时，reset的会???陙E
...
volatile unsigned char running_timer;//特殊??枚ㄊ逼丒
...
volatile unsigned char speed_min_avg;//记录平???〈偶?
volatile unsigned char speed_min_array[16];//记录先前?据
volatile unsigned char start_delay;
volatile unsigned int velocity;
volatile unsigned char life[3];//备用，记录寿脕E

unsigned int rampup_timer;//?动延迟时间，?动+10g
...
volatile union 
{
	...
	struct
	{
		unsigned motor_running :1;	    //电?状态位
		unsigned tx_300 :1;			//接收信号处?蛠E疚?
		unsigned rx_300 :1;			//AD转换处?蛠E疚?
		unsigned port_check :1;		//端口查询处?蛠E疚?
		unsigned ad_conv :1;			//
		...
		unsigned cutter_down :1;		//?刀动?眮E疚?
		unsigned adj_start :1;			//张力校准眮E疚?
		unsigned zero_error :1;
		unsigned special_start :1;		//?零蛠E?
		unsigned speedlow :1;
	}BIT;	
}flag1;
...
volatile union
{
	...
	struct
	{
		...
		unsigned l_1 :1;
		unsigned l_2 :1;
		unsigned command :1;			//传送字节中的状态位（??当前字节是?是脕E?坠是?据?
		unsigned zero_end :1;
...
void __attribute__((__interrupt__, auto_psv)) _T1Interrupt(void)//TMR1每50uS?葋E酥卸?
{
	IFS0bits.T1IF = 0; //Clear Timer1 interrupt flag
	if(tx_timer!=0)
	{
		tx_timer--;//125*13.2uS=1.65mS处??淮蝦
	}
	else
	{
		tx_timer=32;
	...
	if(check_timer != 0) 
	{
		check_timer --;;//19*13.2uS=250uS处??淮?
	}
	else
	{
		check_timer=4;
	...
	if(display_timer!=0) 
	{
		display_timer--;//(999+1)*50us=50mS处??淮蝔
	}
	else
	{
		display_timer=999;
...
void __attribute__((__interrupt__, auto_psv)) _CNInterrupt(void)//?于6000转电?，?位秮E00us?葋E淮沃卸?
{
	//int k;
...
LATA = 0x0000;//写 LATx 寄存?觼E?PORTx 寄存?的效?相同
LATB = 0x0000;//写 LATx 寄存?觼E?PORTx 寄存?的效?相同
//PORT_CUTTER = 0;//防止?始化以??刀?动讈E
...
/******************** intializes I2C ******************************/
I2C1BRG = 0x0012; 	//First set the I2C(1) BRG Baud Rate.计算值为10，改成12?余量
...
T1CONbits.TON = 1; // Start Timer,50us计时?开
...
while(1)
{
	...
	if(flag1.BIT.lowvolt==1)						//如?电压低就?葋E缪共檠?
	{
		while(!PORT_BOR)						//等?电压恢复
		goto loop;				
	}
	if(flag1.BIT.ad_conv==1)						//AD计藖E输?值改写
	{
		//PORT_CUTTER=1;
		flag1.BIT.ad_conv=0;
		motor_control();//debug
	...
	if(flag1.BIT.rx_300==1)						//3.3mS一次的?据接收
	{
		flag1.BIT.rx_300=0;
		data_rx();
	}
	if(flag1.BIT.tx_300==1)						//1.65mS一次的?据发送
	{
		flag1.BIT.tx_300=0;
		data_tx();
	}
...
void check_port(void)
{
	...
	else */if(flag1.BIT.rx_start == 0)	//电压?常,紒E橥ㄑ?
	{
		if(PORT_RX == 0)				//如?RA4有低电平
		{
			rx_timer = 33;				//?1.65mS再紒E丒
			rx_bitcntr = 0;
			rx_waitcntr = 17;
			flag1.BIT.rx_300 = 0;
...
测试发现：以下程?使用时间为8us(电?未动)，10us(电?运转普通緛E
*************************************************************************/
void GetSpeed(void)
{
	if ((speed_cntr1 > 3) && (flag1.BIT.cutter_down == 0))// 600RPM:12pu/50ms;24pu/100ms//原5?磁极 0902
	{
...
/**************************************************************************************/
		...
		//每50ms循环一次,200?循环为10S，判断1次??值和??≈担丒滤俣炔???
		if(++special_cycletimer == 200)
		{
			...
			if(start_delay > 10)//延迟10?周期
			{
				//start_delay = 10;//使变量保持不眮E
				speed_min_avg = (unsigned char)(speed_min_sum >> 3);//取平???〈偶丒
			}
			else //运转?期，?淹速度取10脕E谧?淹值，之?取平??
			{
				start_delay++;
	...
	//??3：超时结?特殊緛E
	//??4：特殊?怍动延迟
		if((speed_delta > special_error_set) && (speed_deltalast > special_error_set) && (flag3.BIT.special_wind == 1))//3次判断，?延迟?用
		{
	...
	else //if (speed_timer > 0)//速度计??在3000以内，说?电?在运转，切换?葋E缁?运转状?
	{
        flag1.BIT.motor_running = 0;				//开始?动电?时眮E疚晃?
		//pwm_out_temp = 0;
		pwm_out = 0;
		pwm_add = 0;
		flag1.BIT.special_start = 0;
		...
		cnt_FIR++;
		cnt_FIR = cnt_FIR & 0x07;
		speed_min_array[cnt_FIR] = 0;//清?之前保存的??〈偶?楷防止由于运算?导致的清羴E
...
//特殊?喂偿?据时间来定，
//每50ms紒E庖淮嗡俣龋∷俣茸?淹点?为?始点，?0ms取张力偏??担迫丒娲⑵??
//共200?存储?，10脕E奔洹?
//?据下一周期的时间点，取?对应的张力偏??担?PI运算，?为补偿值，加葋E怂恪?
//tens_error_s[special_timer]记录上一次该时间点的偏瞾E
//tens_addt[special_timer]记录补偿值，?量式补偿法
	if(flag1.BIT.special_start == 1)
	{
		...
		tens_addt_temp = tens_comp + tens_complast[running_timer+1] - tens_complast[running_timer];//PID,张力补偿值?量式PI算法
		tens_addt[running_timer] += (tens_addt_temp / 8);
		tens_complast[running_timer] = tens_comp;
		//tens_add = tens_addt[running_timer];
		tens_add = tens_addt[running_timer];//特殊?圬制有延迟，当补偿了以?，需要0.5脕E拍芷???于是提前补偿200ms以?的张力偏瞾E
	}

	//velocity = speed_cntr1 * 25; //48相位/圈, 速度=50ms?冲?*20*60/48，20次为1脕E?0脕E分钟
...
void pulse_count(void)
{
	...
	{
		pulse_cntr --;
		//length_compen --;				//如?为6圈1米?每25米补1米（6.5圈）
		//if(length_compen <= 0)
		//{
		//	length_compen = 625;
...
{	unsigned char i;
	...
	else if(flag1.BIT.adj_start == 1)//张力范围?节状态
	{
		if(tens_temp < 780)
		{ led_mode(5); }
	...
	else
	{
		...
		{
			case 0 :
				if(flag3.BIT.wind_end == 1)//?淦结蕘E
				{ led_mode(3); }
				else if(flag3.BIT.wind_start == 1)//?淦?程中
				{
...
/*	if(WR==1)//查EEPROM寄存?是?忙
	...
	else
	{
			...
			{
				case 0 : 	if(setlen[0] > 5)//长度末位?非0时为特殊緛E
							{
				...
				case 1 : 	data_backup1();
							eeprom_address ++;
							break;
				case 2 : 	eeprom_address ++;//Nop();			//延迟一?周期
							break;
				case 3 : 	eeprom_address ++;//Nop();			//延迟一?周期
							break;
	...
	if(flag1.BIT.motor_running == 0)
	{
		...
		if(rx_bytecntr == 5)
		{
			...
			if(c == rx_buffer[4])
			{
				...
				eeprom_address = 0;
				rx_bytecntr = 0;
				flag2.byte = 1;					//reset的处??号
				flag3.byte = 0;					//reset的处??号
				//?零准备
				OC1R = 0;// Write the duty cycle for the first PWM pulse
                pwm_zero = 0;// Write the duty cycle for the second PWM pulse
				pwm_out_temp = 0;
				zero_delay = 5;
				flag1.BIT.adj_start = 0;			//?苈丒?零眮E疚?
				flag1.BIT.zero_error = 0;			//清??零??的信息
				//flag1.BIT.reset_end = 1;			//防止未?零就?动
...
void rxadj(void)
{
	if(flag1.BIT.motor_running == 0)//电?静止状态
	{
		if(rx_bytecntr == 0)
		{
			//rx_bytecntr = 0;
			flag2.byte = 2;			//senadj的处??号
			//?零准备
			OC1R = 0;// Write the duty cycle for the first PWM pulse
            pwm_zero = 0;// Write the duty cycle for the second PWM pulse
			zero_delay = 5;
...
void data_rx(void)
{
	...
	if(flag1.BIT.rx_start == 1)		//开始接收?据
	{
		rx_bitcntr ++;
		if(rx_bitcntr == 10)
		{
...
void tension_compensate(void)
{
	...
	tens_set = settens;               //?控板在发送时乘以2，?以当设定100克时，settens的值是200
	if(tens_set > 200)//100g为??值
	{
		tens_set = 200;
	}
	else if(tens_set < 10)
	...
	t_backup = tens_set*4; //(tens_set*8)/2;	//1g = 8?AD值,100g的AD总值为800
...
/**************************************AD转换子程衼E**************************************/
/*************************************电?控制子程衼E*************************************/
/***********************24us停止，??运行状态,long=65us,int=35us*************************/
/*****************************************************************************************/
void motor_control(void)
{
	...
	if(flag1.BIT.motor_running == 1)
	{
		...
		tens_deviation = tens_temp - tens_desired;//当前?瞾E

		/*if(rampup_timer != 0)	//for start, 10 sec, add tension
		{
			rampup_timer --;
			tens_deviation = tens_deviation -80;//?加10g张力
...
/**********************************************************************************/
	...
	if(Button_in == 0) //2023膩E糜赽utton控制电极转动
	{
		pwm_out_temp = 200;
		pwm_out = 200;	
	}
