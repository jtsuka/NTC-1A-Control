...
volatile union 
{
	...
	struct
	{
		...
		unsigned tx_300 :1;			//接收信号处理标志位
		unsigned rx_300 :1;			//AD转换处理标志位
		unsigned port_check :1;		//端口查询处理标志位
		unsigned ad_conv :1;			//
		unsigned rx_start :1;			//接收信号标志位
		unsigned display :1;			//LED显示标志位
		unsigned lowvolt :1;			//低电压循环标志位
		unsigned speed_drop :1;			//电机制动标志位
		unsigned speed_count :1;		//速度计算标志位
		unsigned pulse_count :1;		//丝长计算标志位
		unsigned cutter_down :1;		//剪刀动作标志位	
		unsigned adj_start :1;			//张力校准标志位
		unsigned zero_error :1;
		unsigned special_start :1;		//调零完毕
		unsigned speedlow :1;
	}BIT;	
}flag1;
...
测试发现：以下程序使用时间为8us(电机未动)，10us(电机运转普通卷）
*************************************************************************/
void GetSpeed(void)
{
	if ((speed_cntr1 > 3) && (flag1.BIT.cutter_down == 0))// 600RPM:12pu/50ms;24pu/100ms//原5个磁极 0902
	{
		...
		else if(speed_cntr1 < (speed_min_avg + 4))//判断速度是否接近转折点。
		{
		...
		//每50ms循环一次,200个循环为10S，判断1次最大值和最小值，更新速度差值
		if(++special_cycletimer == 200)
		{
			special_cycletimer = 0;
			//speed_deltaprev = speed_deltalast;
			speed_deltalast = speed_delta;//赋值，更新上一次速度差值
			speed_minlast = speed_min;//保存当前最低速度，下一次10秒周期使用
			speed_delta = speed_max - speed_min;//更新此次速度差值
			...
			{
				//start_delay = 10; //使变量保持不变
				speed_min_avg = (unsigned char)(speed_min_sum >> 3); //取平均最小磁极差
			}
			else //运转初期，最低速度取10秒内最低值，之后取平均数
			{
				start_delay++;
	...
	//条件3：超时结束特殊卷
	//条件4：特殊卷启动延迟
		if((speed_delta > special_error_set) && (speed_deltalast > special_error_set) && (flag3.BIT.special_wind == 1))//速度差大于预设值，启动特殊卷
		{
	...
	else //if (speed_timer > 0)//速度计数器在3000以内，说明电机在运转，切换进入电机运转状态
	{
        flag1.BIT.motor_running = 0;				///开始驱动电机时标志位为1
		//pwm_out_temp = 0;
		pwm_out = 0;
		pwm_add = 0;
		flag1.BIT.special_start = 0;
		...
		cnt_FIR++;
		cnt_FIR = cnt_FIR & 0x07;
		speed_min_array[cnt_FIR] = 0;////清除之前保存的最小磁极差，防止由于运算而导致的清

		speed_min_sum = 0;
		speed_min_avg = 0;//记录平均最小磁极数
		start_delay = 0;//启动延迟
...
//特殊卷补偿根据时间来定，
//每50ms检测一次速度，取速度最低点作为起始点，每50ms取张力偏差值，计入存储器。
//共200个存储器，10秒时间。
//根据下一周期的时间点，取出对应的张力偏差值，做PI运算，作为补偿值，加入运算。
//tens_error_s[special_timer]记录上一次该时间点的偏差
//tens_addt[special_timer]记录补偿值，增量式补偿法
	if(flag1.BIT.special_start == 1)
	{
		...
		tens_add = tens_addt[running_timer];//特殊卷控制有延迟，当补偿了以后，需要0.5秒才能起作用.于是提前补偿200ms以后的张力偏差
	}

	//velocity = speed_cntr1 * 25; //48相位/圈, 速度=50ms脉冲数*20*60/48，20次为1秒，60秒1分钟
...
void pulse_count(void)
{
	if(flag3.BIT.wind_end == 0)
	{
		...
		if(pulse_cntr == 0)
		{
			...
			pulse_cntr = 300;					//50mm直径，51*3.14/圈,48相位/圈，1米299.7个相位切换
		}
	}
}
/*************************************led红绿模式子程序***********************************************/
/* 输入参数：1：绿灯闪。2：绿灯亮。3：红绿灯同时亮。4：红灯亮。5：红灯闪。6：红绿灯交替闪。7：红灯高速闪。8：红灯高速闪 */
/*******************************************************************************************************/
void led_mode(unsigned char led_chose)
{
	if(led_chose == 1)
...
void motor_control(void)
{
	...
	if(flag1.BIT.motor_running == 1)
	{
		...
		tens_deviation = tens_temp - tens_desired;//当前偏差。
		...
		else//filling running
		{
			...
			pwm_add = 0.029 * (tens_deviation - last_deviation) + Ki * tens_deviation + Kd * (tens_deviation + prev_deviation - 2 * last_deviation);
		}
	...
	{stop_delay=1000;} //timing=500ms(500us x 1000)
