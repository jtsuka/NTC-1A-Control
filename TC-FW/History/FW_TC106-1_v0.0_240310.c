...
//set GCP_ON
#pragma config JTAGEN = OFF       // JTAG 無効
#pragma config FWDTEN = OFF       // WDT 無効
#pragma config ICS = PGx2         // ピン設定
#pragma config FCKSM = CSDCMD     // クロックスイッチ無効、WDT無効
#pragma config POSCMOD = NONE     // 外部発振器なし
...
#define pid_scale 1000 //1600ﾊｱ,110T-96-262,ﾈﾕｱｾﾊﾔﾑ・600
/*************************************************************
...
*************************************************************/
//const unsigned int StateLoTable[8] = {0x0000, 0x0210, 0x2004, 0x0204, 0x0801, 0x0810, 0x2001, 0x2A00};//Version2.0,ﾓｲｼ?ｱ荳・ﾄﾐ?
//const unsigned int StateLoTable[8] = {0x0000, 0x0021, 0x0018, 0x0009, 0x0006, 0x0024, 0x0012, 0x0000};//ｵ扈?ｷｴﾗｪ｣ｬｱ・-6ｸﾄﾎｪ6-1｣ｬｴﾋｶﾎﾈ｡ﾏ?ﾁﾋｵﾗｶﾋPWM｡｣
//const unsigned int StateLoTable[8] = {0x0000, 0x2001, 0x0810, 0x0801, 0x0204, 0x2004, 0x0210, 0x2A00};//ｵ扈?ｷｴﾗｪ｣ｬｱ・-6ｸﾄﾎｪ6-1｣ｬｵﾗｶﾋPWM
//const unsigned int StateLoTable[8] = {0x0000, 0x0120, 0x1008, 0x0108, 0x0402, 0x0420, 0x1002, 0x0000};//ｸﾟｶﾎPWM

const unsigned int StateLoTable[8] = {0x0000, 0x8400, 0x1800, 0x9000, 0x6000, 0x2400, 0x4800, 0x0000};//ｵ扈?ｻｻﾏ?ｱ・
...
volatile unsigned char setlen[3];		//ﾉ雜ｨｵﾄｳ､ｶﾈ｣ｨｶﾔﾋ・?ﾐﾐｼ?ｷｨﾊｵﾏﾖｳ､ｶﾈｿﾘﾖﾆ｣?
volatile signed char len_1[3];		//ﾊ｣ﾓ犒ﾄｳ､ｶﾈ,ｼﾆﾋ羚｢ﾉ?ｸｺﾊ?
volatile signed char lastlen[3];		//ｳ､ｶﾈ｣ｨﾒﾑｾｭｾ?茖ｵﾄｳ､ｶﾈ
volatile unsigned char settens;
volatile unsigned int tens_set;			//ﾉ雜ｨﾕﾅﾁｦﾖｵ
volatile unsigned char txtens[2];			//ｷ｢ﾋﾍｵﾄﾕﾅﾁｦﾖｵ
volatile signed char tmp_1;
volatile signed char tmp_2;

volatile unsigned int tens_desired;		//ﾔﾋﾋ耨ﾃﾉ雜ｨﾕﾅﾁｦﾖｵ
volatile unsigned int tens_actual;		//ｴ豺ﾅﾕﾅﾁｦﾆｽｾ?ﾖｵ
//volatile unsigned int tens_sample[16];	//A/Dｽ盪?ｴ豺ﾅｵﾄｱ菽ｿ
volatile unsigned int tens_sum;			//ｴ豺ﾅ16ｴﾎｲﾉｼｯｵﾄADﾖｵｵﾄﾗﾜｺﾍ
volatile unsigned int tens_zero;		    //ｴ豺ﾅﾁ羞耻ﾅﾁｦﾖｵ
volatile unsigned int tens_temp;
volatile signed int tens_deviation;			//ﾎ?ｲ??ｵ
volatile signed int last_deviation;		//ﾇｰｴﾎﾎ?ｲ・
volatile signed int prev_deviation;		//ﾉﾏｴﾎﾎ?ｲ・
...
volatile unsigned int rx_timer;		//ｽﾓﾊﾕﾊｱｼ莊ﾄｼﾆﾊ?ﾆ・
...
volatile unsigned char rx_buffer[6];			//ｽﾓﾊﾕﾊ?ｾﾝﾇ・
volatile unsigned int tx_timer;			//ｷ｢ﾋﾍﾊｱｼ莊ﾄｼﾆﾊ?ﾆ・
...
volatile unsigned int stop_delay;			//ｵ扈?ﾍ｣ﾖｹﾊｱｼ・
volatile unsigned int cutter_timer;		//ｼ?ｵｶｸｴﾎｻｼﾆﾊ?ﾆ・

volatile unsigned int check_timer;			//ｶﾋｿﾚｲ鰉ｯﾊｱｼ莨ﾆﾊ?ﾆ・
volatile signed int pulse_cntr;				//ﾋｿｳ､ｼﾆﾊ?,ﾔﾚｼ?ｷｨﾔﾋﾋ飜ｱﾐ靨ｪｸｺﾊ?

volatile unsigned int display_timer;				//LEDﾉﾁﾋｸﾑﾓｳﾙｼﾆﾊ?
volatile unsigned int ledcntr;				//LEDﾉﾁﾋｸﾑﾓｳﾙｼﾆﾊ?
volatile unsigned int HallValue;
volatile unsigned int eeprom_address;	//ｱ｣ｴ贇ﾚeepromﾖﾐｵﾄｸ?ﾊ?
volatile unsigned char zero_delay;
//ｵ?ﾊﾔ
volatile signed int tens_addt[data_total];//ｼﾇﾂｼｲｻﾍｬﾊｱｼ荼ﾎｵﾄｲｹｳ･ﾖｵ
//2014.8.24ｸﾄﾎｪ190ｸ?ﾊ?ｾﾝ,200ｸ?ﾊ?ｾﾝﾎｪｼｫﾏﾞ,ﾔﾙﾔ?ｼﾓｱ菽ｿﾊｱ｣ｬresetｵﾄｻ盍?ﾉ?ｴ?黶E
volatile signed int tens_complast[data_total];//ｼﾇﾂｼｲｻﾍｬﾊｱｼ荼ﾎｵﾄﾕﾅﾁｦﾆｫｲ・
...
volatile unsigned char running_timer;//ﾌﾘﾊ篝??ﾃｶｨﾊｱﾆ・
...
volatile unsigned char speed_min_avg;//ｼﾇﾂｼﾆｽｾ?ﾗ??｡ｴﾅｼｫﾊ?
volatile unsigned char speed_min_array[16];//ｼﾇﾂｼﾏﾈﾇｰﾊ?ｾﾝ
volatile unsigned char start_delay;
volatile unsigned int velocity;
volatile unsigned char life[3];//ｱｸﾓﾃ｣ｬｼﾇﾂｼﾊﾙﾃ・

unsigned int rampup_timer;//ﾆ?ｶｯﾑﾓｳﾙﾊｱｼ茱ｬﾆ?ｶｯ+10g
...
volatile union 
{
	...
	struct
	{
		unsigned motor_running :1;	    //ｵ扈?ﾗｴﾌｬﾎｻ
		unsigned tx_300 :1;			//ｽﾓﾊﾕﾐﾅｺﾅｴｦﾀ?ﾍ・ｾﾎ?
		unsigned rx_300 :1;			//ADﾗｪｻｻｴｦﾀ?ﾍ・ｾﾎ?
		unsigned port_check :1;		//ｶﾋｿﾚｲ鰉ｯｴｦﾀ?ﾍ・ｾﾎ?
		unsigned ad_conv :1;			//
		unsigned rx_start :1;			//ｽﾓﾊﾕﾐﾅｺﾅｱ・ｾﾎ?
		unsigned display :1;			//LEDﾏﾔﾊｾｱ・ｾﾎ?
		unsigned lowvolt :1;			//ｵﾍｵ醵ｹﾑｭｻｷｱ・ｾﾎ?
		unsigned speed_drop :1;		//used in future for brake
		unsigned speed_count :1;		//ﾋﾙｶﾈｼﾆﾋ羈・ｾﾎ?
		unsigned pulse_count :1;		//ﾋｿｳ､ｼﾆﾋ羈・ｾﾎ?
		unsigned cutter_down :1;		//ｼ?ｵｶｶｯﾗ?ｱ・ｾﾎ?
		unsigned adj_start :1;			//ﾕﾅﾁｦﾐ｣ﾗｼｱ・ｾﾎ?
		unsigned zero_error :1;
		unsigned special_start :1;		//ｵ?ﾁ耋・?
		unsigned speedlow :1;
	}BIT;	
}flag1;
...
volatile union
{
	...
	struct
	{
		...
		unsigned l_1 :1;
		unsigned l_2 :1;
		unsigned command :1;			//ｴｫﾋﾍﾗﾖｽﾚﾖﾐｵﾄﾗｴﾌｬﾎｻ｣ｨｱ?ﾟ?ｵｱﾇｰﾗﾖｽﾚﾊﾇｷ?ﾊﾇﾃ・?ﾗｹﾊﾇﾊ?ｾﾝ｣?
		unsigned zero_end :1;
...
void __attribute__((__interrupt__, auto_psv)) _T1Interrupt(void)//TMR1ﾃｿ50uSｽ?ﾈ・ﾋﾖﾐｶ?
{
	IFS0bits.T1IF = 0; //Clear Timer1 interrupt flag
	if(tx_timer!=0)
	{
		tx_timer--;//125*13.2uS=1.65mSｴｦﾀ??ｻｴﾎ
	}
	else
	{
		tx_timer=32;
	...
	if(rx_timer!=0)
	{
		 rx_timer--;//250*13.2uS=3.3mSｴｦﾀ??ｻｴﾎ
	}
	else
	{
		rx_timer=65;
	...
	if(check_timer != 0) 
	{
		check_timer --;;//19*13.2uS=250uSｴｦﾀ??ｻｴﾎ
	}
	else
	{
		check_timer=4;
	...
	if(display_timer!=0) 
	{
		display_timer--;//(999+1)*50us=50mSｴｦﾀ??ｻｴﾎ
	}
	else
	{
		display_timer=999;
...
void __attribute__((__interrupt__, auto_psv)) _CNInterrupt(void)//ｻ?ﾓﾚ6000ﾗｪｵ扈?｣ｬｲ?ﾎｻｶ・00usｽ?ﾈ・ｻｴﾎﾖﾐｶ?
{
	//int k;
...
void para_initial(void)
{
	...
	cnt_FIR = 0;
	speed_min_sum = 0;
	speed_min_avg = 0;//ｼﾇﾂｼﾆｽｾ?ﾗ??｡ｴﾅｼｫﾊ?
	cnt_FIR = 0;
	stop_delay = 1000;
	start_delay = 0;
	rampup_timer = 0;
...
LATA = 0x0000;//ﾐｴ LATx ｼﾄｴ貳?ﾓ・?PORTx ｼﾄｴ貳?ｵﾄﾐｧｹ?ﾏ猩ｬ
LATB = 0x0000;//ﾐｴ LATx ｼﾄｴ貳?ﾓ・?PORTx ｼﾄｴ貳?ｵﾄﾐｧｹ?ﾏ猩ｬ
//PORT_CUTTER = 0;//ｷﾀﾖｹｳ?ﾊｼｻｯﾒﾔｺ?ｼ?ｵｶﾎ?ｶｯﾗ・
...
AD1CON3 = 0x1F03; // auto Sample time = 31Tad, Tad = 4 Tcy, inverting = 12 Tad｣ｬtotal = 43Tad =11.5us
/******************** intializes I2C ******************************/
I2C1BRG = 0x0012; 	//First set the I2C(1) BRG Baud Rate.ｼﾆﾋ聊ｵﾎｪ10｣ｬｸﾄｳﾉ12ﾁ?ﾓ狠ｿ
...
T1CONbits.TON = 1; // Start Timer,50usｼﾆﾊｱｴ?ｿｪ
...
while(1)
{
	if(flag1.BIT.port_check==1)					//250uSﾒｻｴﾎｵﾄｶﾋｿﾚｼ・・
	{
		flag1.BIT.port_check=0;
		check_port();	
	}
	if(flag1.BIT.lowvolt==1)						//ﾈ郢?ｵ醵ｹｵﾍｾﾍｽ?ﾈ・醵ｹｲ鰉ｯﾑｭｻ?
	{
		while(!PORT_BOR)						//ｵﾈｴ?ｵ醵ｹｻﾖｸｴ
		goto loop;				
	}
	if(flag1.BIT.ad_conv==1)						//ADｼﾆﾋ・ﾊ莎?ﾖｵｸﾄﾐｴ
	{
		//PORT_CUTTER=1;
		flag1.BIT.ad_conv=0;
		motor_control();//debug
	...
	if(flag1.BIT.rx_300==1)						//3.3mSﾒｻｴﾎｵﾄﾊ?ｾﾝｽﾓﾊﾕ
	{
		flag1.BIT.rx_300=0;
		data_rx();
	}
	if(flag1.BIT.tx_300==1)						//1.65mSﾒｻｴﾎｵﾄﾊ?ｾﾝｷ｢ﾋﾍ
	{
		flag1.BIT.tx_300=0;
		data_tx();
	}
	if(flag1.BIT.display==1)					//50msﾒｻｴﾎｵﾄLEDﾖｸﾊｾ
	{
		flag1.BIT.display=0;
		led_display();
		//ｼ?ｵｶﾑﾓﾊｱｳﾌﾐ・
	...
	if(flag1.BIT.pulse_count==1)					//ﾏﾟｳ､ｼﾆﾋ・
	{
...
void check_port(void)
{
	/*if(PORT_BOR == 0)						//ｵ醵ｹﾆｫｵﾍ
	{
		INTCON1bits.NSTDIS = 0;				//ｹﾘﾖﾐｶﾏ
		flag1.BIT.lowvolt = 1;				//ﾖﾃｵﾍｵ醵ｹｱ・ｾﾎ?
		P1DC1 = 0;// set PWM 1, 2 and 3 to 0
		P1DC2 = 0;
		P1DC3 = 0;
		LED_RED = 1;								//ﾃ?ｵﾆ,ｲ簗ﾔ｣ｬｵﾆﾁﾁ
		LED_GRN = 1;
		//AD1CON1bits.ADON = 0;			//ｹﾘｱﾕAD
		data_backup1();
		INTCON1bits.NSTDIS = 1;			//ｿｪﾖﾐｶﾏ	
		
	}
	else */if(flag1.BIT.rx_start == 0)	//ｵ醵ｹﾕ?ｳ｣,ｼ・鯱ｨﾑ?
	{
		if(PORT_RX == 0)				//ﾈ郢?RA4ﾓﾐｵﾍｵ酥ｽ
		{
			rx_timer = 33;				//ｹ?1.65mSﾔﾙｼ・・
			rx_bitcntr = 0;
			rx_waitcntr = 17;
			flag1.BIT.rx_300 = 0;
...
ｲ簗ﾔｷ｢ﾏﾖ｣ｺﾒﾔﾏﾂｳﾌﾐ?ﾊｹﾓﾃﾊｱｼ萸ｪ8us(ｵ扈?ﾎｴｶｯ)｣ｬ10us(ｵ扈?ﾔﾋﾗｪﾆﾕﾍｨｾ・
*************************************************************************/
void GetSpeed(void)
{
	if ((speed_cntr1 > 3) && (flag1.BIT.cutter_down == 0))// 600RPM:12pu/50ms;24pu/100ms//ﾔｭ5ｸ?ｴﾅｼｫ 0902
	{
...
/**********************************20200322 updated************************************/
		...
		if((speed_cntr1 + 10) < speed_min_avg) //判断速度下降，停机判定.
		{
			flag1.BIT.speed_drop = 1;
		}
		else if(speed_cntr1 < (speed_min_avg + 4))//判断速度是否接近?折点.
		{
...
/**************************************************************************************/
		...
		//ﾃｿ50msﾑｭｻｷﾒｻｴﾎ,200ｸ?ﾑｭｻｷﾎｪ10S｣ｬﾅﾐｶﾏ1ｴﾎﾗ?ﾐ?ﾖｵｺﾍﾗ??｡ﾖｵ｣ｬｸ・ﾂﾋﾙｶﾈｲ???
		if(++special_cycletimer == 200)
		{
			special_cycletimer = 0;
			//speed_deltaprev = speed_deltalast;
			speed_deltalast = speed_delta;//ｸｳﾖｵ｣ｬｸ・ﾂﾉﾏﾒｻｴﾎﾋﾙｶﾈｲ???
			speed_minlast = speed_min;//ｱ｣ｴ豬ｱﾇｰﾗ?ﾑﾍﾋﾙｶﾈ｣ｬﾏﾂﾒｻｴﾎ10ﾃ・ﾜﾆﾚﾊｹﾓ?
			speed_delta = speed_max - speed_min;//ｸ・ﾂｴﾋｴﾎﾋﾙｶﾈｲ???
			...
			if(start_delay > 10)//ﾑﾓｳﾙ10ｸ?ﾖﾜﾆﾚ
			{
				//start_delay = 10;//ﾊｹｱ菽ｿｱ｣ｳﾖｲｻｱ・
				speed_min_avg = (unsigned char)(speed_min_sum >> 3);//ﾈ｡ﾆｽｾ?ﾗ??｡ｴﾅｼｫｲ・
			}
			else //ﾔﾋﾗｪｳ?ﾆﾚ｣ｬﾗ?ﾑﾍﾋﾙｶﾈﾈ｡10ﾃ・ﾚﾗ?ﾑﾍﾖｵ｣ｬﾖｮｺ?ﾈ｡ﾆｽｾ?ﾊ?
			{
				start_delay++;
	...
	//ﾌﾘﾊ籌ﾋﾐﾐﾅﾐｶﾏ
	//ﾌ?ｼ?1｣ｺﾋﾙｶﾈﾗ?ﾐ?ﾖｵ-ﾗ??｡ﾖｵ<=5ﾂ?ｳ・
	//ﾌ?ｼ?2｣ｺﾋﾙｶﾈｵﾍﾓﾚ600ﾗｪｽ睫?ﾌﾘﾊ篝・
	//ﾌ?ｼ?3｣ｺｳｬﾊｱｽ睫?ﾌﾘﾊ篝・
	//ﾌ?ｼ?4｣ｺﾌﾘﾊ篝?糜ｶｯﾑﾓｳﾙ
		if((speed_delta > special_error_set) && (speed_deltalast > special_error_set) && (flag3.BIT.special_wind == 1))//3ｴﾎﾅﾐｶﾏ｣ｬﾆ?ﾑﾓｳﾙﾗ?ﾓﾃ
		{
	...
	else //if (speed_timer > 0)//ﾋﾙｶﾈｼﾆﾊ?ﾆ?ﾔﾚ3000ﾒﾔﾄﾚ｣ｬﾋｵﾃ?ｵ扈?ﾔﾚﾔﾋﾗｪ｣ｬﾇﾐｻｻｽ?ﾈ・扈?ﾔﾋﾗｪﾗｴﾌ?
	{
        flag1.BIT.motor_running = 0;				//ｿｪﾊｼﾇ?ｶｯｵ扈?ﾊｱｱ・ｾﾎｻﾎ?
		//pwm_out_temp = 0;
		pwm_out = 0;
		pwm_add = 0;
		flag1.BIT.special_start = 0;
		special_cycletimer = 0;//ｱ｣ｳﾖ
		speed_max=0;
		speed_min=255;
		speed_delta = 0;
		speed_deltalast = 0;
		...
		cnt_FIR++;
		cnt_FIR = cnt_FIR & 0x07;
		speed_min_array[cnt_FIR] = 0;//ﾇ蟲?ﾖｮﾇｰｱ｣ｴ豬ﾄﾗ??｡ｴﾅｼｫｲ?ｿｬｷﾀﾖｹﾓﾉﾓﾚﾔﾋﾋ羝?ｵｼﾖﾂｵﾄﾇ蠱・

		speed_min_sum = 0;
		speed_min_avg = 0;//ｼﾇﾂｼﾆｽｾ?ﾗ??｡ｴﾅｼｫﾊ?
		start_delay = 0;//ﾆ?ｶｯﾑﾓｳﾙ
...
//ﾌﾘﾊ篝?ﾎｹｳ･ｸ?ｾﾝﾊｱｼ菎ｴｶｨ｣ｬ
//ﾃｿ50msｼ・簫ｻｴﾎﾋﾙｶﾈ｣ｬﾈ｡ﾋﾙｶﾈﾗ?ﾑﾍｵ聆?ﾎｪﾆ?ﾊｼｵ罐ｬﾃ?0msﾈ｡ﾕﾅﾁｦﾆｫｲ??ｵ｣ｬｼﾆﾈ・豢｢ﾆ?｡?
//ｹｲ200ｸ?ｴ豢｢ﾆ?｣ｬ10ﾃ・ｱｼ茖?
//ｸ?ｾﾝﾏﾂﾒｻﾖﾜﾆﾚｵﾄﾊｱｼ莊罐ｬﾈ｡ｳ?ｶﾔﾓｦｵﾄﾕﾅﾁｦﾆｫｲ??ｵ｣ｬﾗ?PIﾔﾋﾋ罐ｬﾗ?ﾎｪｲｹｳ･ﾖｵ｣ｬｼﾓﾈ・ﾋﾋ罍?
//tens_error_s[special_timer]ｼﾇﾂｼﾉﾏﾒｻｴﾎｸﾃﾊｱｼ莊羞ﾄﾆｫｲ・
//tens_addt[special_timer]ｼﾇﾂｼｲｹｳ･ﾖｵ｣ｬﾔ?ﾁｿﾊｽｲｹｳ･ｷｨ
	if(flag1.BIT.special_start == 1)
	{
		...
		if(flag1.BIT.speedlow == 1)//ﾗ?ﾑﾍｵ・
		{
			//PORT_CUTTER^=1;//ｵ?ﾊﾔﾓﾃ
			running_timer = 0;
		}
		tens_addt_temp = tens_comp + tens_complast[running_timer+1] - tens_complast[running_timer];//PID,ﾕﾅﾁｦｲｹｳ･ﾖｵﾔ?ﾁｿﾊｽPIﾋ羚ｨ
		tens_addt[running_timer] += (tens_addt_temp / 8);
		tens_complast[running_timer] = tens_comp;
		//tens_add = tens_addt[running_timer];
		tens_add = tens_addt[running_timer];//ﾌﾘﾊ篝?ﾛﾘﾖﾆﾓﾐﾑﾓｳﾙ｣ｬｵｱｲｹｳ･ﾁﾋﾒﾔｺ?｣ｬﾐ靨ｪ0.5ﾃ・ﾅﾄﾜﾆ?ﾗ?ﾓ?ﾓﾚﾊﾇﾌ睇ｰｲｹｳ･200msﾒﾔｺ?ｵﾄﾕﾅﾁｦﾆｫｲ・
	}

	//velocity = speed_cntr1 * 25; //48ﾏ猥ｻ/ﾈｦ, ﾋﾙｶﾈ=50msﾂ?ｳ衞?*20*60/48｣ｬ20ｴﾎﾎｪ1ﾃ・?0ﾃ・ｷﾖﾖﾓ
...
void pulse_count(void)
{
	...
	{
		pulse_cntr --;
		//length_compen --;				//ﾈ郢?ﾎｪ6ﾈｦ1ﾃﾗﾔ?ﾃｿ25ﾃﾗｲｹ1ﾃﾗ｣ｨ6.5ﾈｦ｣ｩ
		//if(length_compen <= 0)
		//{
		//	length_compen = 625;
		...
		if(pulse_cntr == 0)
		{
			...
			pulse_cntr = 338;					//pulse_cntr = 48 x 1000 / (dia.45.2 x 3.14) = 338.2
		}
	}
}
/*************************************ledｺ・ﾌﾄ｣ﾊｽﾗﾓｳﾌﾐ・**********************************************/
/*ﾊ菠・ﾎﾊ?｣?｣ｺﾂﾌｵﾆﾉﾁ｡｣2｣ｺﾂﾌｵﾆﾁﾁ｡｣3｣ｺｺ・ﾌｵﾆﾍｬﾊｱﾁﾁ｡?｣ｺｺ・ﾆﾁﾁ｡?｣ｺｺ・ﾆﾉﾁ｡?｣ｺｺ・ﾌｵﾆｽｻﾌ賈ﾁ｡?｣ｺｺ・ﾆｸﾟﾋﾙﾉﾁ｡?｣ｺｺ・ﾆｸﾟﾋﾙﾉ?*/
/*******************************************************************************************************/
void led_mode(unsigned char led_chose)
{
	if(led_chose == 1)
...
{	unsigned char i;
	...
	else if(flag1.BIT.adj_start == 1)//ﾕﾅﾁｦｷｶﾎｧｵ?ｽﾚﾗｴﾌｬ
	{
		if(tens_temp < 780)
		{ led_mode(5); }
	...
	else
	{
		...
		{
			case 0 :
				if(flag3.BIT.wind_end == 1)//ｾ?菷ｽ睫・
				{ led_mode(3); }
				else if(flag3.BIT.wind_start == 1)//ｾ?菷ｹ?ｳﾌﾖﾐ
				{
...
/*	if(WR==1)//ｲ颱EPROMｼﾄｴ貳?ﾊﾇｷ?ﾃｦ
	...
	else
	{
			...
			{
				case 0 : 	if(setlen[0] > 5)//ｳ､ｶﾈﾄｩﾎｻﾊ?ｷﾇ0ﾊｱﾎｪﾌﾘﾊ篝・
							{
				...
				case 1 : 	data_backup1();
							eeprom_address ++;
							break;
				case 2 : 	eeprom_address ++;//Nop();			//ﾑﾓｳﾙﾒｻｸ?ﾖﾜﾆﾚ
							break;
				case 3 : 	eeprom_address ++;//Nop();			//ﾑﾓｳﾙﾒｻｸ?ﾖﾜﾆﾚ
							break;
				case 4 : 	data_backup2();			//ﾉ雜ｨｳ､ｶﾈﾖﾐﾁｽﾎｻ
							eeprom_address ++;
							break;
	...
	if(flag1.BIT.motor_running == 0)
	{
		...
		if(rx_bytecntr == 5)
		{
			...
			if(c == rx_buffer[4])
			{
				...
				pulse_cntr = 143;
				settens=rx_buffer[3];
				tension_compensate();           //ﾕﾅﾁｦｲｹｳ･/ｼﾆﾋ・
				tens_desired = tens_set;
				eeprom_address = 0;
				rx_bytecntr = 0;
				flag2.byte = 1;					//resetｵﾄｴｦﾀ?ﾐ?ｺﾅ
				flag3.byte = 0;					//resetｵﾄｴｦﾀ?ﾐ?ｺﾅ
				//ｵ?ﾁ聆ｼｱｸ
				OC1R = 0;// Write the duty cycle for the first PWM pulse
                pwm_zero = 0;// Write the duty cycle for the second PWM pulse
				pwm_out_temp = 0;
				zero_delay = 5;
				flag1.BIT.adj_start = 0;			//?ﾜﾂ・?ﾁ羈・ｾﾎ?
				flag1.BIT.zero_error = 0;			//ﾇ蟲?ｵ?ﾁ羔??ｵﾄﾐﾅﾏ｢
				//flag1.BIT.reset_end = 1;			//ｷﾀﾖｹﾎｴｵ?ﾁ羸ﾍﾆ?ｶｯ
...
void rxadj(void)
{
	if(flag1.BIT.motor_running == 0)//ｵ扈?ｾｲﾖｹﾗｴﾌｬ
	{
		if(rx_bytecntr == 0)
		{
			//rx_bytecntr = 0;
			flag2.byte = 2;			//senadjｵﾄｴｦﾀ?ﾐ?ｺﾅ
			//ｵ?ﾁ聆ｼｱｸ
			OC1R = 0;// Write the duty cycle for the first PWM pulse
            pwm_zero = 0;// Write the duty cycle for the second PWM pulse
			zero_delay = 5;
...
void data_rx(void)
{
	...
	if(flag1.BIT.rx_start == 1)		//ｿｪﾊｼｽﾓﾊﾕﾊ?ｾﾝ
	{
		rx_bitcntr ++;
		if(rx_bitcntr == 10)
		{
...
void tx_cntrset(void)
{
	...
	if(tx_bitcntr == 17)
	{
		...
		if(tx_bytecntr == 6)//ｽ?ﾐﾐｳ､ｶﾈｼ?ｷｨﾔﾋﾋ・
		{
			tx_bytecntr = 0;
			tmp_1 = setlen[1];
			tmp_2 = setlen[2];
...
void tension_compensate(void)
{
	...
	tens_set = settens;               //ﾖ?ｿﾘｰ袂ﾚｷ｢ﾋﾍﾊｱｳﾋﾒﾔ2｣ｬﾋ?ﾒﾔｵｱﾉ雜ｨ100ｿﾋﾊｱ｣ｬsettensｵﾄﾖｵﾊﾇ200
	if(tens_set > 200)//100gﾎｪﾗ?ﾐ?ﾖｵ
	{
		tens_set = 200;
	}
	else if(tens_set < 10)
	...
	t_backup = tens_set*4; //(tens_set*8)/2;	//1g = 8ｸ?ADﾖｵ,100gｵﾄADﾗﾜﾖｵﾎｪ800
	t_backup = t_backup - 16;	//ﾈ･2ｿﾋﾗ霖ｦ
	tens_set = t_backup;
...
/**************************************ADﾗｪｻｻﾗﾓｳﾌﾐ・**************************************/
/*************************************ｵ扈?ｿﾘﾖﾆﾗﾓｳﾌﾐ・*************************************/
/***********************24usﾍ｣ﾖｹ｣ｬﾗ?ﾐ?ﾔﾋﾐﾐﾗｴﾌｬ,long=65us,int=35us*************************/
/*****************************************************************************************/
void motor_control(void)
{
	...
	if(flag1.BIT.motor_running == 1)
	{
		...
		tens_deviation = tens_temp - tens_desired;//ｵｱﾇｰﾎ?ｲ・

		/*if(rampup_timer != 0)	//for start, 10 sec, add tension
		{
			rampup_timer --;
			tens_deviation = tens_deviation -80;//ﾔ?ｼﾓ10gﾕﾅﾁｦ
...
/**********************************************************************************/
	...
	if(Button_in == 0) //2023ﾄ・ﾃﾓﾚbuttonｿﾘﾖﾆｵ郛ｫﾗｪｶｯ
	{
		pwm_out_temp = 200;
		pwm_out = 200;	
	}
	else if(tens_temp < 35) //tension <5g, brake timing
	{
		if(stop_delay != 0)
		{stop_delay--;}
		else
